...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanareasignado=global_popup_IWTSystem(ventanareasignado,"nuevoreasignado.asp?vistapadre=" + window.name + "&paginapadre=admreasignado.asp&codreasignado=" + codigo,"Newreasignado","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 350)/2 - 30) + ",height=350,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanareasignado=global_popup_IWTSystem(ventanareasignado,"nuevoreasignado.asp?vistapadre=" + window.name + "&paginapadre=admreasignado.asp","Newreasignado","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 350)/2 - 30) + ",height=350,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function abrircargatxt()
		{
			ventanacargatxt=global_popup_IWTSystem(ventanacargatxt,"cargarreasignado.asp?vistapadre=" + window.name + "&paginapadre=admreasignado.asp","NewCargaReasignado","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=150,width=" + (screen.width/3 + 30) + ",left=" + (screen.width/4 + 60) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Código','Agencia','Código Central','Caduca','Segmento Gestión','Act','Editar');
		    identificadorfilas[tabla]="fila";
		    pievisible[tabla]=true;
		    columnavisible[tabla] = new Array(true, true,true , true , true,true , true );
		    anchocolumna[tabla] =  new Array( '4%',      '20%' , '8%' ,    '8%',    '8%',      '3%' ,   '20%');
		    aligncabecera[tabla] = new Array('left','left','left','left','left','left','left');
		    aligndetalle[tabla] = new Array('left','left','left','left','left','left','left');
		    alignpie[tabla] =     new Array('left','left','left','left','left','left','left');
		    decimalesnumero[tabla] = new Array(-1 ,-1      ,-1   ,-1 ,-1     ,-1 ,-1);
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codreasignado-id- value=-c0->' + '<a href="javascript:modificar(-id-);">-valor-</a>';//+ objetodatos("text",tabla,"usuario","left","6","");
				...
				objetofomulario[tabla][5]=objetodatos("checkbox",tabla,"activo","","","");
				objetofomulario[tabla][6]='<a href=javascript:modificar("-id-");>Editar</a>';
		...
		<%
		if buscador<>"" then
			filtrobuscador = " where (B.razonsocial like '%" & buscador & "%' or A.codigocentral like '%" & buscador & "%' or A.fechacaduca like '%" & buscador & "%' or A.segmento_gestion like '%" & buscador & "%') "
		end if
		...
		contadortotal=0
		sql="select count(*) from reasignado A inner join agencia B on A.codagencia = B.codagencia " & filtrobuscador 
		consultar sql,RS	
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " A.codreasignado,A.codigocentral, A.fechacaduca, A.activo, B.razonsocial as agencia,A.segmento_gestion " & _
			"from reasignado A inner join agencia B on A.codagencia = B.codagencia where " & filtrobuscador1 & " A.codreasignado not in (select top " & topnovisible & " A.codreasignado from reasignado A inner join agencia B on A.codagencia = B.codagencia " & filtrobuscador & " order by A.codreasignado desc) order by A.codreasignado desc" 
		else
		sql="select top " & cantidadxpagina & " A.codreasignado,A.codigocentral, A.fechacaduca, A.activo, B.razonsocial as agencia,A.segmento_gestion " & _
			"from reasignado A inner join agencia B on A.codagencia = B.codagencia " & filtrobuscador & " order by A.codreasignado desc" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF
				if obtener("actualizarlista")<>"" and obtener("codreasignado" & RS.Fields("codreasignado"))<>"" then
					...
					if obtener("activo" & RS.Fields("codreasignado"))<>"" then
						activo="1"
					else
						activo="0"
					end if						
						...
						if int(activo)<> rs.Fields("activo") then
							...
							'''sql="update agencia set razonsocial='" & razonsocial & "',telefono='" & telefono & "',direccion='" & dieccion & "',activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codreasignado=" & rs.Fields("codreasignado") 
							sql="update reasignado set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codreasignado=" & rs.Fields("codreasignado") 
							''response.write sql
							conn.Execute sql
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]=<%=RS.Fields("codreasignado")%>;
				datos[tabla][<%=contador%>][1]='<%=rs.Fields("agencia")%>';
			    datos[tabla][<%=contador%>][2]='<%=rs.Fields("codigocentral")%>';
				datos[tabla][<%=contador%>][3]='<%=rs.Fields("fechacaduca")%>';
				datos[tabla][<%=contador%>][4]='<%=rs.Fields("segmento_gestion")%>';
				datos[tabla][<%=contador%>][5]=<%if obtener("actualizarlista")<>"" and obtener("codreasignado" & RS.Fields("codreasignado"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
				datos[tabla][<%=contador%>][6]='';
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Reasignado (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Reasignado (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%>&nbsp;&nbsp;<a href='javascript:abrircargatxt();'><img src="imagenes/cargar.gif" border=0 alt="Cargar Archivo de Texto" title="Cargar Archivo de Texto" align=middle></a></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Código','Agencia','Código Central','Caduca','Segmento Gestión','Activo' "
					...
					consulta_exp="select A.codreasignado, B.razonsocial,char(39) + A.codigocentral, convert(varchar,A.fechacaduca,103),A.segmento_gestion,CASE WHEN A.Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.reasignado A inner join CobranzaCM.dbo.agencia B on A.codagencia = B.codagencia  " & filtrobuscador & " order by A.codreasignado desc" 
