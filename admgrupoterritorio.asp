...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanagrupoterritorio=global_popup_IWTSystem(ventanagrupoterritorio,"nuevogrupoterritorio.asp?vistapadre=" + window.name + "&paginapadre=admgrupoterritorio.asp&codgrupoterritorio=" + codigo,"NewGrupoterritorio","scrollbars=no,scrolling=no,top=" + ((screen.height - 140)/2 - 30) + ",height=140,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanagrupoterritorio=global_popup_IWTSystem(ventanagrupoterritorio,"nuevogrupoterritorio.asp?vistapadre=" + window.name + "&paginapadre=admgrupoterritorio.asp","NewGrupoterritorio","scrollbars=no,scrolling=no,top=" + ((screen.height - 140)/2 - 30) + ",height=140,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    anchocolumna[tabla] =  new Array(  '5%' , '15%' , '');
		    ...
		    objetofomulario[tabla] = new Array();
				...
				objetofomulario[tabla][1]='<input type=hidden name=codgrupoterritorio-id- value=-c0->' + '<a href="javascript:modificar(-id-);">-valor-</a>';
		...
		contadortotal=0
		sql="select count(*) from Grupoterritorio  " & filtrobuscador 
		consultar sql,RS	
		...
		sql="select top " & cantidadxpagina & " * " & _
			"from Grupoterritorio where " & filtrobuscador1 & " codgrupoterritorio not in (select top " & topnovisible & " codgrupoterritorio from Grupoterritorio " & filtrobuscador & " order by codgrupoterritorio) order by codgrupoterritorio" 
		else
		sql="select top " & cantidadxpagina & " * " & _
			"from Grupoterritorio " & filtrobuscador & " order by codgrupoterritorio" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codgrupoterritorio" & RS.Fields("codgrupoterritorio"))<>"" then
					
					'if obtener("orden" & RS.Fields("codgrupoterritorio"))<>"" then
					'	orden = obtener("orden" & RS.Fields("codgrupoterritorio"))
					'else
					'	orden =""
					'end if
					 ...
					 '	if 	obtener("orden" & RS.Fields("codgrupoterritorio")) <> rs.Fields("orden") then
																										
					 	'	sql="update Grupoterritorio set orden = " & orden & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codgrupoterritorio=" & rs.Fields("codgrupoterritorio") 
					'		''response.write sql
						 '	conn.Execute sql
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]=<%=RS.Fields("codgrupoterritorio")%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Grupo territorio (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Grupo Territorio (<%=contadortotal%>)&nbsp;&nbsp;<!--<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;--><a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=middle width=250><font size=2 face=Arial color=#00529B>Buscar:&nbsp;<input name="buscador" value="<%=buscador%>" size=20 onkeypress="if(window.event.keyCode==13) buscar();"></font></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select descripcion " & _
								 "from CobranzaCM.dbo.grupoterritorio  " & filtrobuscador & " order by codgrupoterritorio" 
