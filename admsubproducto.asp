...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanasubproducto=global_popup_IWTSystem(ventanasubproducto,"nuevosubproducto.asp?vistapadre=" + window.name + "&paginapadre=admsubproducto.asp&codsubproducto=" + codigo,"Newsubproducto","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanasubproducto=global_popup_IWTSystem(ventanasubproducto,"nuevosubproducto.asp?vistapadre=" + window.name + "&paginapadre=admsubproducto.asp","Newsubproducto","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Codigo','Descripción', 'Familia','Producto','Act','Editar');
		    identificadorfilas[tabla]="fila";
		    pievisible[tabla]=true;
		    columnavisible[tabla] = new Array(true, true, true ,true, true,true);
		    anchocolumna[tabla] =  new Array( '5%', '15%', '20%' , '20%','3%' ,'');
		    aligncabecera[tabla] = new Array('center','left','left', 'left','center','left');
		    aligndetalle[tabla] = new Array('center','left','left','left','center','left');
		    alignpie[tabla] =     new Array('left','left','left','left','center','left');
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codsubproducto-id- value=-c0->' + '<a href=javascript:modificar("-id-");>-valor-</a>';
				...
				objetofomulario[tabla][4]=objetodatos("checkbox",tabla,"activo","","","");
				objetofomulario[tabla][5]='<a href=javascript:modificar("-id-");>Editar</a>';
		    ...
		    tipofiltrofomulario[tabla] = new Array();
				...
				filtrofomulario[tabla][3]=''; //objetofiltro("text",tabla,2,'contiene');
		...
		<%
		if buscador<>"" then
			filtrobuscador = " where (f.codsubproducto like '%" & buscador & "%' or f.descripcion like '%" & buscador & "%' or g.descripcion like '%" & buscador & "%' or h.descripcion like '%" & buscador & "%') "
		end if
		...
		contadortotal=0
		sql="select count(*) from subproducto f left outer join producto g on f.codproducto = g.codproducto left outer join famproducto h on f.codfamproducto=h.codfamproducto " & filtrobuscador 
		consultar sql,RS	
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " *,f.descripcion as descrip,g.descripcion as grupo, h.descripcion as codfamproducto, f.activo as activo from subproducto f left outer join producto g on f.codproducto = g.codproducto left outer join famproducto h on f.codfamproducto=h.codfamproducto where " & filtrobuscador1 & " f.codsubproducto not in (select top " & topnovisible & " f.codsubproducto from subproducto f left outer join producto g on f.codproducto = g.codproducto " & filtrobuscador & " order by f.codsubproducto) order by f.codsubproducto" 
		else
		sql="select top " & cantidadxpagina & " *,f.descripcion as descrip,g.descripcion as grupo, h.descripcion as codfamproducto, f.activo  as activo from subproducto f left outer join producto g on f.codproducto = g.codproducto left outer join famproducto h on f.codfamproducto=h.codfamproducto" & filtrobuscador & " order by f.codsubproducto" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codsubproducto" & RS.Fields("codsubproducto"))<>"" then
					
					if obtener("activo" & RS.Fields("codsubproducto"))<>"" then
						activo="1"
					else
						activo="0"
					end if											
						...
						if 	int(activo)<> rs.Fields("activo") then
							sql="update subproducto set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codsubproducto= '" & rs.Fields("codsubproducto")&  "'" 
							''response.write sql
							conn.Execute sql
						end if
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]='<%=RS.Fields("codsubproducto")%>';
				datos[tabla][<%=contador%>][1]='<%=iif(isNull(rs.Fields("descrip")),"",rs.Fields("descrip"))%>';
				datos[tabla][<%=contador%>][2]='<%=rs.Fields("codfamproducto")%>';
				datos[tabla][<%=contador%>][3]='<%=rs.Fields("grupo")%>';
				datos[tabla][<%=contador%>][4]=<%if obtener("actualizarlista")<>"" and obtener("codsubproducto" & RS.Fields("codsubproducto"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>SubProducto (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>SubProducto (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.SubProducto','Descripción', 'Familia', 'Producto','Activo'"
					...
					consulta_exp="select char(39) + f.codsubproducto,f.descripcion,g.descripcion, h.descripcion, CASE WHEN f.Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.subproducto f left outer join CobranzaCM.dbo.producto g on f.codproducto = g.codproducto left outer join CobranzaCM.dbo.famproducto h on f.codfamproducto=h.codfamproducto " & filtrobuscador & " order by f.codsubproducto " 
