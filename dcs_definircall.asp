		...
		<body topmargin="0" leftmargin="0" bgcolor="#FFFFFF">
			<table border="0" cellspacing="0" cellpadding="0" width="100%" height="100%">
				<form name="formula" method="post" action="dcs_definiraccion.asp">					
					<tr class="fondo-red">	
						<td class="text-withe" >			
							<font size="2"><b>&nbsp;<b><%if codfacultad="" then%><%end if%>Definir CALL</b></b></font>	
						</td>
					...
					<tr >
						<td class="text-orange"><font size="2">Campaña:</font></td>
						<td class="text-orange"><font size="2"><%=Nombcampana%><br>Inicio:<%=fechainicio%>&nbsp;al:<%=fechafin%></font></td>
					</tr>
					<tr class="fondo-gris">
						<td class="text-orange"><font size="2">Asignar a:</font></td>
						<td>
							...
							Do While Not  RS.EOF
							%>
								<option value="<%=RS.Fields("CodUsuario")%>" <% if codusuario<>"" then%><% if RS.fields("CodUsuario")=int(codusuario) then%> selected<%end if%><%end if%>><%=RS.Fields("Usuario")%></option>
							<%
							RS.MoveNext
							loop
							RS.Close
