      ...
      function toggleCodes(on) {
		...
		<script language="javascript">
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    anchocolumna[tabla] =  new Array( '7%' , '15%' ,  '4%',   '');
		...
		contadortotal=0
		sql="select count(*) from Perfil  " & filtrobuscador 
		consultar sql,RS	
		...
		sql="select top " & cantidadxpagina & " * " & _
			"from Perfil where " & filtrobuscador1 & " codperfil not in (select top " & topnovisible & " codperfil from Perfil " & filtrobuscador & " order by codperfil) order by codperfil" 
		else
		sql="select top " & cantidadxpagina & " * " & _
			"from Perfil " & filtrobuscador & " order by codperfil" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
		...
		%>
			...
			datos[tabla][<%=contador%>][0]=<%=RS.Fields("codperfil")%>;
			datos[tabla][<%=contador%>][1]='<%=rs.Fields("descripcion")%>';
			datos[tabla][<%=contador%>][2]=<%if obtener("actualizarlista")<>"" and obtener("codperfil" & RS.Fields("codperfil"))<>"" then%><%=orden%><%else%><%=rs.Fields("orden")%><%end if%>;
				...
				<form name="formula" method="post">
					<table width="100%" cellpadding="4" cellspacing="0">	
						<tr class="fondo-orange">
							<td class="text-orange"><font size="2" face="Fira Sans Condensed" ><b>Perfil (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border="0" alt="Nuevo" title="Nuevo" align="middle"></a></td>
