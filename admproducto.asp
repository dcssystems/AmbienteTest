...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanaproducto=global_popup_IWTSystem(ventanaproducto,"nuevoproducto.asp?vistapadre=" + window.name + "&paginapadre=admproducto.asp&codproducto=" + codigo,"Newproducto","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanaproducto=global_popup_IWTSystem(ventanaproducto,"nuevoproducto.asp?vistapadre=" + window.name + "&paginapadre=admproducto.asp","Newproducto","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Código','Descripción','Act','Editar');
		    identificadorfilas[tabla]="fila";
		    pievisible[tabla]=true;
		    columnavisible[tabla] = new Array(true, true, true ,true);
		    anchocolumna[tabla] =  new Array( '2%', '10%', '3%' , '20%');
		    aligncabecera[tabla] = new Array('center','left','left','left');
		    aligndetalle[tabla] = new Array('center','left','left','left');
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codproducto-id- value=-c0->' + '<a href=javascript:modificar("-id-");>-valor-</a>';
		...
		<%
		if buscador<>"" then
			filtrobuscador = " where (p.codproducto like '%" & buscador & "%' or p.descripcion like '%" & buscador & "%') "
		end if
		...
		contadortotal=0
		sql="select count(*) from producto p " & filtrobuscador 
		consultar sql,RS	
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " *,p.descripcion as descrip, p.activo as activo from producto p where " & filtrobuscador1 & " p.codproducto not in (select top " & topnovisible & " p.codproducto from producto p " & filtrobuscador & " order by p.codproducto) order by p.codproducto" 
		else
		sql="select top " & cantidadxpagina & " *,p.descripcion as descrip, p.activo as activo from producto p " & filtrobuscador & " order by p.codproducto" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista") <> "" and obtener("codproducto" & RS.Fields("codproducto")) <>"" then
					
					if obtener("activo" & RS.Fields("codproducto")) <> "" then
						activo="1"
					else
						activo="0"
					end if											
						...
						if 	int(activo)<> rs.Fields("activo") then
							sql="update producto set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codproducto= '" & rs.Fields("codproducto")&  "'" 
							''response.write sql
							conn.Execute sql
						end if
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]='<%=RS.Fields("codproducto")%>';
				datos[tabla][<%=contador%>][1]='<%=rs.Fields("descrip")%>';
				datos[tabla][<%=contador%>][2]=<%if obtener("actualizarlista")<>"" and obtener("codproducto" & RS.Fields("codproducto"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Producto (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Producto (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.Producto','Descripción','Activo' "
					...
					consulta_exp="select char(39) + p.codproducto,p.descripcion, CASE WHEN p.Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.producto p " & filtrobuscador & " order by codproducto" 
