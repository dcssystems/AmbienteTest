...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("cargadireccion.asp") then
			'###############################################
					...
					For Each File In Uploader.Files.Items		
						...
						if bolUpload = true then
							if cuentaarch=0 then
								...
								sql="INSERT INTO CargaArchivo(proceso,rutaarchivo,mailinforme,usuarioregistra,fecharegistra,codestadocarga) VALUES ('direccion','" & RutaFisicaUpload & "\direccion','" & correo & "'," & session("codusuario") & ",getdate(),1)"
								conn.execute sql		
								
								''capturamos el maximo CargaArchivo para el FileName que sera "direcciones(nro).txt"
								...
								File.FileName="direccion" & codcarga & "." & GetFileExtension(File.FileName)
			...
			%>
				<script language=javascript>
					window.open("cargadireccion.asp?agregardato=<%=Uploader.Form("agregardato")%>&flaginactivar=<%=Uploader.Form("flaginactivar")%>&archivotxt=<%=strMessage%>","_self");
				</script>
			<%
