...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("admlogcarga.asp") then
	...
	%>
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Codigo','Proceso','Ruta','Estado','Usuario','Agencia','Registro','Observaciones','Inicio','Fin','Desc');
		    ...
		    anchocolumna[tabla] =  new Array( '2%', '3%', '20%' , '7%' ,'3%', '4%', '4%' , '3%' ,'3%','3%','3%');
		    aligncabecera[tabla] = new Array('left','left','left','left','center','center','left','center','left','left','left');
		    aligndetalle[tabla] = new Array('left','left','left','left','center','center','left','center','left','center','center');
		    alignpie[tabla] =     new Array('left','left','left','left','center','center','left','center','left','left','left');
		    ...
		    objetofomulario[tabla] = new Array();
				...
				objetofomulario[tabla][10]='<a href=javascript:abrirarchivo("-valor-");><img src="imagenes/descargarpeq.png" border=0 alt="Descargar Archivo" title="Descargar Archivo"></a>';
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " d.razonsocial, a.codcarga, a.proceso, a.rutaarchivo, b.descripcion, c.usuario, convert(varchar,a.fecharegistra,103)as fecharegistra, a.observaciones, a.fechainicio, a.fechafin from cargaarchivo a inner join estadocarga b on a.codestadocarga = b.codestadocarga inner join usuario c on a.usuarioregistra = c.codusuario left outer join agencia d on c.codagencia = d.codagencia where " & filtrobuscador1 & " a.codcarga not in (select top " & topnovisible & " a.codcarga from cargaarchivo a inner join estadocarga b on a.codestadocarga = b.codestadocarga inner join usuario c on a.usuarioregistra = c.codusuario " & filtrobuscador & " order by a.codcarga desc) order by a.codcarga desc" 
		else
		sql="select top " & cantidadxpagina & " d.razonsocial, a.codcarga, a.proceso, a.rutaarchivo, b.descripcion, c.usuario, convert(varchar,a.fecharegistra,103)as fecharegistra, a.observaciones, a.fechainicio, a.fechafin from cargaarchivo a inner join estadocarga b on a.codestadocarga = b.codestadocarga inner join usuario c on a.usuarioregistra = c.codusuario left outer join agencia d on c.codagencia = d.codagencia " & filtrobuscador & " order by a.codcarga desc" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
		...
		%>
				...
				datos[tabla][<%=contador%>][6]='<%=RS.Fields("fecharegistra")%>';
				datos[tabla][<%=contador%>][7]='<%=RS.Fields("observaciones")%>';
				datos[tabla][<%=contador%>][8]='<%=RS.Fields("fechainicio")%>';
				datos[tabla][<%=contador%>][9]='<%=RS.Fields("fechafin")%>';
				datos[tabla][<%=contador%>][10]='<%=nombrearchivo%>';
		...
		''Si se pide exportar a excel
				...
				''Codigo','Proceso','Ruta','Estado','Usuario','Fecha registro','Observaciones','Fecha inicio','Fecha fin');
					...
					consulta_exp="select a.codcarga, a.proceso, a.rutaarchivo, b.descripcion, c.usuario, convert(varchar,a.fecharegistra,103), a.observaciones, convert(varchar,a.fechainicio,109), convert(varchar,a.fechafin,109)" & _
