...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanatipousuario=global_popup_IWTSystem(ventanatipousuario,"nuevotipousuario.asp?vistapadre=" + window.name + "&paginapadre=admtipousuario.asp&codtipousuario=" + codigo,"NewTipoUsuario","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 160)/2 - 30) + ",height=160,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanatipousuario=global_popup_IWTSystem(ventanatipousuario,"nuevotipousuario.asp?vistapadre=" + window.name + "&paginapadre=admtipousuario.asp","NewTipoUsuario","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 160)/2 - 30) + ",height=160,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    anchocolumna[tabla] =  new Array( '5%' , '15%' , '3%','');
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codtipousuario-id- value=-c0-><a href="javascript:modificar(-id-);">-valor-</a>';
		...
		case "0" : 
					...
					filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where "," and ") & "Activo=0"
		case "2" : 
					checkbuscactivos="value='2'"
					filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where "," and ") & "Activo=1"
		case else: 
					checkbuscactivos="value='1'"
		end select		
		
		contadortotal=0
		sql="select count(*) from TipoUsuario  " & filtrobuscador 
		consultar sql,RS	
		...
		sql="select top " & cantidadxpagina & " * " & _
			"from TipoUsuario where " & filtrobuscador1 & " codtipousuario not in (select top " & topnovisible & " codtipousuario from TipoUsuario " & filtrobuscador & " order by codtipousuario) order by codtipousuario" 
		else
		sql="select top " & cantidadxpagina & " * " & _
			"from TipoUsuario " & filtrobuscador & " order by codtipousuario" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codtipousuario" & RS.Fields("codtipousuario"))<>"" then
					
					if obtener("activo" & RS.Fields("codtipousuario"))<>"" then
						activo="1"
					else
						activo="0"
					end if	
						...
						if int(activo)<> rs.Fields("activo") then
							...
							sql="update TipoUsuario set activo = " & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codtipousuario=" & rs.Fields("codtipousuario") 
							''response.write sql
							conn.Execute sql
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]=<%=RS.Fields("codtipousuario")%>;
			    datos[tabla][<%=contador%>][1]='<%=rs.Fields("descripcion")%>';
				datos[tabla][<%=contador%>][2]=<%if obtener("actualizarlista")<>"" and obtener("codtipousuario" & RS.Fields("codtipousuario"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Tipo Usuario (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Tipo Usuarios (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.TipUsuario','Descripción','Activo'"
					...
					consulta_exp="select codtipousuario,descripcion,CASE WHEN Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.tipousuario " & filtrobuscador & " order by codtipousuario" 
