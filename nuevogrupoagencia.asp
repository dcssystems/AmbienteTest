...
if session("codusuario")<>"" then
	...
	codgrupoagencia=obtener("codgrupoagencia")
		...
		descripcion=obtener("descripcion")
			...
			existegrupoagencia=0
			
			if codgrupoagencia<>"" then
			sql="select count(*) from grupoagencia where descripcion='" & descripcion & "' and codgrupoagencia <>" & codgrupoagencia 
			else
			sql="select count(*) from grupoagencia where descripcion='" & descripcion & "'"
			end if
			consultar sql,RS
			existegrupoagencia = RS.Fields(0)
			RS.Close			
			if existegrupoagencia=0 then			
				if obtener("agregardato")="1" then		
					sql="insert into GrupoAgencia (descripcion,usuarioregistra,fecharegistra) values ('" & descripcion & "'," & session("codusuario") & ",getdate())"
				else
					sql="update grupoAgencia set descripcion='" & descripcion & "',usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codgrupoagencia=" & codgrupoagencia
				end if
				''Response.Write sql
				conn.execute sql
				...
				<script language=javascript>
					...
					<%end if%>				
					<%if obtener("paginapadre")="admgrupoagencia.asp" then%>window.open("<%=obtener("paginapadre")%>","<%=obtener("vistapadre")%>");<%end if%>
					window.close();
				</script>			
				<%
			else
			...
			if codgrupoagencia<>"" then
					sql="select A.*,B.nombres as Nombreusureg, B.apepaterno as Apepatusureg, B.apematerno as Apematusureg, C.nombres as Nombreusumod,C.apepaterno as Apepatusumod, C.apematerno as Apematusumod from grupoagencia A inner join usuario B on B.codusuario=A.usuarioregistra left outer join usuario C on C.codusuario=A.usuariomodifica where a.codgrupoagencia = " & codgrupoagencia
					consultar sql,RS
		...
		<html>
			<title><%if codgrupoagencia="" then%>Nuevo <%end if%>Grupo Agencia</title>
			...
			<body topmargin=0 leftmargin=0 bgcolor="#FFFFFF">
					...
					<form name=formula method=post action="nuevogrupoagencia.asp">
					<tr height=20>	
						<td bgcolor="#F5F5F5" colspan=2>			
							<font size=2 color=#483d8b face=Arial><b>&nbsp;<b><%if codgrupoagencia="" then%>Nuevo <%end if%>Grupo Agencia</b></b></font>
						</td>
					</tr>
					<%if fechaReg<>"" then%>
					<tr height=20>
					...
					<tr>					
						...
						<td bgcolor="#F5F5F5" align=right height=40><%if codgrupoagencia="" then%><a href="javascript:agregar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar"></a>&nbsp;<%else%><a href="javascript:modificar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar"></a>&nbsp;<%end if%><a href="javascript:window.close();"><img src="imagenes/salida.gif" border=0 alt="Salir" title="Salir"></a>&nbsp;</td>					
					</tr>
						<input type=hidden name="agregardato" value="">
						<input type=hidden name="codgrupoagencia" value="<%=codgrupoagencia%>">
