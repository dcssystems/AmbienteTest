...
if session("codusuario")<>"" then
	...
	if permisofacultad("admimpagado.asp") then
		...
		''sql="select getdate()+1"
		''consultar sql,RS	
		''maxfechagestion=rs.fields(0)
		''RS.Close

		''sql="select getdate()-7"
		''consultar sql,RS	
		''minfechagestion=rs.fields(0)
		''RS.Close
		...
		sql="SELECT DD.codigocentral, DD.contrato, DD.fechadatos, DD.codagencia, DD.nombre, DD.telefono, DD.tipofono1, DD.prefijo1, DD.fono1, DD.extension1, DD.codproducto," &_ 
                   "DD.codsubproducto, SP.descripcion AS SubProducto, PR.descripcion AS Producto, DD.diasvencimiento, DD.divisa, DD.saldohoy," &_
                   "AG.razonsocial AS Agencia, DD.fechagestion, DD.tipodocumento, DD.numdocumento, DD.codterritorio, DD.codoficina, DD.direccion, DD.codestado, DD.codprovincia," &_
                   "DD.coddistrito, UB.departamento, UB.provincia, UB.distrito, CL.tipofono2, CL.prefijo2, CL.fono2, CL.extension2, CL.tipofono3, CL.prefijo3, CL.fono3, CL.extension3," &_ 
                   "CL.tipofono4, CL.prefijo4, CL.fono4, CL.extension4, CL.tipofono5, CL.prefijo5, CL.fono5, CL.extension5, CL.email, DD.fechaincumplimiento, " &_
                          "MC.codmarca, " &_
                          "MA.descripcion AS marca " &_
              "FROM         dbo.BusquedaDistribucion AS DD INNER JOIN " &_ 
                      "dbo.ClienteDiario AS CL ON DD.codigocentral = CL.codigocentral AND DD.fechadatos = CL.fechadatos INNER JOIN " &_
                      "dbo.Agencia AS AG ON DD.codagencia = AG.codagencia LEFT OUTER JOIN " &_
                      "dbo.Producto AS PR ON PR.codproducto = DD.codproducto LEFT OUTER JOIN " &_
                      "dbo.SubProducto AS SP ON SP.codsubproducto = DD.codsubproducto LEFT OUTER JOIN " &_
                      "dbo.Ubigeo AS UB ON UB.coddpto = CL.codestado AND UB.codprov = CL.codprovincia AND UB.coddist = CL.coddistrito " &_
             ...
             "WHERE DD.codigocentral='" & codigocentral & "' and DD.fechadatos='" & mid(obtener("fechadatos"),7,4) & mid(obtener("fechadatos"),4,2) & mid(obtener("fechadatos"),1,2) & "' order by DD.diasvencimiento desc,DD.saldohoy desc" 
             
sql="SELECT DD.codigocentral, DD.contrato, DD.fechadatos, DD.codagencia, DD.nombre, DD.telefono, DD.tipofono1, DD.prefijo1, DD.fono1, DD.extension1, DD.codproducto," &_ 
                   "DD.codsubproducto, SP.descripcion AS SubProducto, IsNull(FP.descripcion,'') AS Producto, DD.diasvencimiento, DD.divisa, DD.saldohoy," &_
                   "AG.razonsocial AS Agencia, DD.fechagestion, DD.tipodocumento, DD.numdocumento, DD.codterritorio, DD.codoficina, DD.direccion, DD.codestado, DD.codprovincia," &_
                   "DD.coddistrito, UB.departamento, UB.provincia, UB.distrito, CL.tipofono2, CL.prefijo2, CL.fono2, CL.extension2, CL.tipofono3, CL.prefijo3, CL.fono3, CL.extension3," &_ 
                   "CL.tipofono4, CL.prefijo4, CL.fono4, CL.extension4, CL.tipofono5, CL.prefijo5, CL.fono5, CL.extension5, CL.email, DD.fechaincumplimiento, " &_
                          "MC.codmarca, " &_
              ...
              "FROM         dbo.BusquedaDistribucion AS DD INNER JOIN " &_ 
                      ...
                      "dbo.Agencia AS AG ON DD.codagencia = AG.codagencia LEFT OUTER JOIN " &_
                      "dbo.SubProducto AS SP ON SP.codsubproducto = DD.codsubproducto LEFT OUTER JOIN " &_
                      "dbo.FamProducto AS FP ON FP.codfamproducto = SP.codfamproducto LEFT OUTER JOIN " &_
                      "dbo.Ubigeo AS UB ON UB.coddpto = CL.codestado AND UB.codprov = CL.codprovincia AND UB.coddist = CL.coddistrito " &_
                      "left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 left outer join Marca MA ON MC.codmarca=MA.codmarca " &_
             "WHERE DD.codigocentral='" & codigocentral & "' and DD.fechadatos='" & mid(obtener("fechadatos"),7,4) & mid(obtener("fechadatos"),4,2) & mid(obtener("fechadatos"),1,2) & "' order by DD.diasvencimiento desc,DD.saldohoy desc"              
		''Response.Write sql
		...
		%>	
			...
			Do While not RS1.EOF
			    if listacontratos="" then
			        listacontratos="'" & RS1.Fields("Contrato") & "'"
			    else
			        listacontratos=listacontratos & ",'" & RS1.Fields("Contrato") & "'"
			    end if
            RS1.MoveNext
			Loop
			RS1.MoveFirst
            ...
            'CARGO CUOTAS
			sql="select * from CuotaDiario where contrato in (" & listacontratos & ") and fechadatos='" & year(RS1.Fields("FechaDatos")) & right("00" & month(RS1.Fields("FechaDatos")),2) & right("00" & day(RS1.Fields("FechaDatos")),2) & "' order by fechavencimiento,divisa"
		    ''response.Write sql			
			consultar sql,RS2			
			'FIN CARGO CUOTAS
			...
			Do While not RS1.EOF
                ...
                RS2.Filter=" contrato='" & RS1.Fields("Contrato") & "' "
				nrocuotas=0
				DivisaDif=""
				MontoTotalDivisa=0
							...
							<td valign="top" align="center" colspan=7>
								<table cellspacing=2 cellpadding=2 border=0 width="95%">
									<tr bgcolor="#BEE8FB">
										...
										<td align="center" width="19%" <%if DivisaDif<>"" then%> colspan="2"<%end if%>><font size=1 face=Arial color=#00529B>Gast/Com/Seg/Otros</font></td>
									...
									<tr bgcolor="#BEE8FB">
										...
										<td align="right"><font size=1 face=Arial color=#00529B><%=FormatNumber(TotalCapital1 + TotalInteres1 + TotalComision1 + TotalInteresMora1,2)%><%''MontoTotalDivisa%></font></td>
										<%if DivisaDif<>"" then%><td align="right"><font size=1 face=Arial color=#00529B><%=FormatNumber(TotalCapital2 + TotalInteres2 + TotalComision2 + TotalInteresMora2,2)%><%''MontoTotalDivisaDif%></font></td><%end if%>
									</tr>									
								</table>
							</td>
					</tr>							
