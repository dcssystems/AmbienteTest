...
if session("codusuario")<>"" then
	...
	sql="select max(fechadatos) from Cliente_FichaVisita"
	consultar sql,RS	
...
<script language=javascript src="scripts/TablaDinamica.js"></script>
		...
		{
		    ventanaverficha = window.open("verficha.asp?vistapadre=" + window.name + "&paginapadre=admvisitas.asp&fdcodcen=" + fdcodcen, "VerFicha" + fdcodcen, "scrollbars=yes,scrolling=yes,top=" + ((screen.height) / 2 - 300) + ",height=600,width=" + (screen.width / 2 + 300) + ",left=" + (screen.width / 2 - 475) + ",resizable=yes");
...
<%Response.Flush()%>
	...
	<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    aligndetalle[tabla] = new Array('left','left','left','left','left','left','left','left','left','left','right','left','left');
		...
		if contrato<>"" then
			filtrobuscador = filtrobuscador & " and A.codigocentral in (select codigocentral from Contrato_FichaVisita where fechadatos=A.fechadatos and contrato='" & contrato & "')"
		end if
		...
		if codproducto<>"" then
			filtrobuscador = filtrobuscador & " and A.codigocentral in (select codigocentral from Contrato_FichaVisita where codproducto='" & codproducto & "' and fechadatos=A.fechadatos)"
		end if		
		if codmarca<>"" then
		...
		contadortotal=0
		sql="select count(*) from Cliente_FichaVisita A inner join Detalle_FichaVisita B on A.codigocentral=B.codigocentral and A.fechadatos=B.fechadatos " & filtrobuscador
		''response.write sql
		consultar sql,RS	
		...
		if pag>1 then
		    sql="select top " & cantidadxpagina & " convert(varchar,A.fechadatos,112) + A.codigocentral as FDCli,(select descripcion from EstadoVisita where codestado=B.estado) as estado,(select descripcion from Clasificacion where codclasificacion=A.clasificacion) as clasifica,A.* from Cliente_FichaVisita A inner join Detalle_FichaVisita B on A.codigocentral=B.codigocentral and A.fechadatos=B.fechadatos where " & filtrobuscador1 & " convert(varchar,A.fechadatos,112) + A.codigocentral not in (select top " & topnovisible & " convert(varchar,A.fechadatos,112) + A.codigocentral  from Cliente_FichaVisita A inner join Detalle_FichaVisita B on A.codigocentral=B.codigocentral and A.fechadatos=B.fechadatos " & filtrobuscador & " order by convert(varchar,A.fechadatos,112) + A.codigocentral) order by convert(varchar,A.fechadatos,112) + A.codigocentral"
		else
		    sql="select top " & cantidadxpagina & " convert(varchar,A.fechadatos,112) + A.codigocentral as FDCli,(select descripcion from EstadoVisita where codestado=B.estado) as estado,(select descripcion from Clasificacion where codclasificacion=A.clasificacion) as clasifica,A.* from Cliente_FichaVisita A inner join Detalle_FichaVisita B on A.codigocentral=B.codigocentral and A.fechadatos=B.fechadatos " & filtrobuscador & " order by convert(varchar,A.fechadatos,112) + A.codigocentral"
		end if
		''response.write sql
		consultar sql,RS
...
<table border=0 cellspacing=0 cellpadding=2 width=100%>
  <tr bgcolor="#007DC5">
    <td colspan="17" align="center" height="22">
      <font size=2 face=Arial color="#FFFFFF"><b>Gestión de Impagados - Visitas de Campo</b></font>
    </td>
  </tr>
  <tr bgcolor="#E9F8FE">
    ...
    <td colspan="4">
		<select name="codgestor" style="font-size: x-small; width: 250px;">
				...
				<%
				sql = "select A.codusuario,A.nombres,A.apepaterno,A.apematerno from Usuario A inner join TipoUsuario B on A.codtipousuario=B.codtipousuario and B.descripcion='Gestor' and A.codusuario in (select distinct codgestor from Detalle_FichaVisita) and A.activo=1"
				consultar sql,RS
				Do While Not  RS.EOF
  ...
  <tr bgcolor="#E9F8FE">
    ...
    <td colspan="4">
			<select name="codtipodocumento" style="font-size: x-small; width: 250px;">
				...
				<%
				sql = "select codtipodocumento,descripcion from TipoDocumento where activo=1 and codtipodocumento in (select distinct tipodocumento from Cliente_FichaVisita) order by descripcion"
				consultar sql,RS
				Do While Not RS.EOF
				%>
    ...
    <select name="segmento" style="font-size: x-small; width: 250px;">
				...
				<%
				sql = "select distinct SEGMENTO_RIESGO from Cliente_FichaVisita order by SEGMENTO_RIESGO"
				consultar sql,RS
				Do While Not  RS.EOF
				%>
  ...
  <tr bgcolor="#E9F8FE">
    ...
    <td colspan="4">
        <select name="codproducto" style="font-size: x-small; width: 250px;">
				...
				<%
				sql = "select codproducto, descripcion from producto where activo=1 and codproducto in (select distinct codproducto from Contrato_FichaVisita) order by codproducto"
				consultar sql,RS
				Do While Not  RS.EOF
				%>
    ...
    <td colspan="4">
    		<select name="codmarca" style="font-size: x-small; width: 250px;">
				...
				<%
				sql = "select codmarca,descripcion from Marca where activo=1 and descripcion in (select distinct marca from Cliente_FichaVisita) order by codmarca"
				consultar sql,RS
				Do While Not  RS.EOF
				%>
  ...
  <tr bgcolor="#E9F8FE">
    ...
    <td colspan="4">
		...
		<select name="codterritorio" onchange="actualizaterritorio()" style="font-size: x-small; width: 250px;">
			...
			<%
				sql = "select distinct codterritorio, territorio as descripcion from Cliente_FichaVisita order by codterritorio"
				consultar sql,RS
				Do While Not  RS.EOF
				%>
    ...
    <td colspan="4">
		...
		<%else%>        
			...
			<%
				if codterritorio<>"" then
					sql = "select distinct codoficina, oficina as descripcion from Cliente_FichaVisita where codterritorio = " & codterritorio & " order by codoficina"
					consultar sql,RS
					Do While Not  RS.EOF
					%>
  ...
  <td colspan="17">
		 <%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=1 face=Arial color=#00529B><b>Fichas de Visita (0) - No hay registros.</b></font>&nbsp;<a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
			</tr>
			</table>
		<%else		
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=1 face=Arial color=#00529B><b>Fichas de Visita (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select convert(varchar,A.fechadatos,103),char(39) + A.codigocentral,A.nombres,A.TipoDocumento + ' - ' + A.NumDocumento,A.Marca,A.Segmento_Riesgo,A.Banca,A.codterritorio + ' - ' + A.territorio,A.codoficina + ' - ' + A.oficina,A.maxdias,(select descripcion from CobranzaCM.dbo.EstadoVisita where codestado=B.estado) as estado,(select descripcion from CobranzaCM.dbo.Clasificacion where codclasificacion=A.clasificacion) as clasifica " & _
								"from CobranzaCM.dbo.Cliente_FichaVisita A inner join CobranzaCM.dbo.Detalle_FichaVisita B on A.codigocentral=B.codigocentral and A.fechadatos=B.fechadatos " & replace(filtrobuscador,"from ","from CobranzaCM.dbo.") & " order by convert(varchar,A.fechadatos,112) + A.codigocentral"
