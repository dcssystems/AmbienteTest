...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanaexcluido=global_popup_IWTSystem(ventanaexcluido,"nuevoexcluido.asp?vistapadre=" + window.name + "&paginapadre=admexcluido.asp&codexcluido=" + codigo,"Newexcluido","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanaexcluido=global_popup_IWTSystem(ventanaexcluido,"nuevoexcluido.asp?vistapadre=" + window.name + "&paginapadre=admexcluido.asp","Newexcluido","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function abrircargatxt()
		{
			ventanacargatxt=global_popup_IWTSystem(ventanacargatxt,"cargarexcluido.asp?vistapadre=" + window.name + "&paginapadre=admexcluido.asp","NewCargaExcluido","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=150,width=" + (screen.width/3 + 30) + ",left=" + (screen.width/4 + 60) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Código','Código Central','Contrato','Act','Editar');
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codexcluido-id- value=-c0->' + '<a href=javascript:modificar("-id-");>-valor-</a>';
		...
		<%
		if buscador<>"" then
			filtrobuscador = " where (f.codexcluido like '%" & buscador & "%' or f.codigocentral like '%" & buscador & "%' or f.contrato like '%" & buscador & "%') "
		end if
		...
		contadortotal=0
		sql="select count(*) from excluido f " & filtrobuscador 
		consultar sql,RS	
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " * from excluido f where " & filtrobuscador1 & " f.codexcluido not in (select top " & topnovisible & " f.codexcluido from excluido f " & filtrobuscador & " order by f.codexcluido desc) order by f.codexcluido desc" 
		else
		sql="select top " & cantidadxpagina & " * from excluido f " & filtrobuscador & " order by f.codexcluido desc" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codexcluido" & RS.Fields("codexcluido"))<>"" then
					
					if obtener("activo" & RS.Fields("codexcluido"))<>"" then
						activo="1"
					else
						activo="0"
					end if											
						...
						if 	int(activo)<> rs.Fields("activo") then
							sql="update excluido set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codexcluido= '" & rs.Fields("codexcluido")&  "'" 
							''response.write sql
							conn.Execute sql
						end if
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]='<%=RS.Fields("codexcluido")%>';
				datos[tabla][<%=contador%>][1]='<%=rs.Fields("codigocentral")%>';
				datos[tabla][<%=contador%>][2]='<%=iif(isNull(rs.Fields("contrato")),"",rs.Fields("contrato"))%>';
				datos[tabla][<%=contador%>][3]=<%if obtener("actualizarlista")<>"" and obtener("codexcluido" & RS.Fields("codexcluido"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Excluidos (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Excluidos (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%>&nbsp;&nbsp;<a href='javascript:abrircargatxt();'><img src="imagenes/cargar.gif" border=0 alt="Cargar Archivo de Texto" title="Cargar Archivo de Texto" align=middle></a></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Código','Código Central','Contrato','Activo' "
					...
					consulta_exp="select f.codexcluido, char(39) + f.codigocentral,char(39) +  f.contrato,CASE WHEN f.Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.excluido f  " & filtrobuscador & " order by f.codexcluido desc" 
