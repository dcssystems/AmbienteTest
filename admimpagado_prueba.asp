...
<%Response.Flush()%>
	...
	<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    anchocolumna[tabla] =  new Array( '2%' , '1%' ,  '7%',  '2%',  '2%','2%' , '1%' ,  '2%',  '1%',  '5%','1%' , '1%' ,  '7%',  '4%',  '',  '');
		...
		filtrobuscador = " where A.fechagestion between convert(datetime,'" & mid(fechaasigini,7,4) & mid(fechaasigini,4,2) & mid(fechaasigini,1,2) & "') and convert(datetime,'" & mid(fechaasigfin ,7,4) & mid(fechaasigfin,4,2) & mid(fechaasigfin,1,2) & "')"
		if contrato<>"" then
		...
		if codtipodocumento<>"" then
			filtrobuscador = filtrobuscador & " and A.codtipodocumento='" & codtipodocumento & "'"
		end if		
		if numdocumento<>"" then
			filtrobuscador = filtrobuscador & " and A.numdocumento='" & numdocumento & "'"
		end if		
		if diaatrasoini<>"" then
			filtrobuscador = filtrobuscador & " and A.diasvencimiento>=" & diaatrasoini
		end if				
		if diaatrasofin<>"" then
			filtrobuscador = filtrobuscador & " and A.diasvencimiento<=" & diaatrasofin
		end if						
		if codproducto<>"" then
			filtrobuscador = filtrobuscador & " and A.codproducto='" & codproducto & "'"
		end if								
		if codagencia<>"" then
			filtrobuscador = filtrobuscador & " and A.codagencia=" & codagencia
		end if										
		if codmarca<>"" then
			filtrobuscador = filtrobuscador & " and A.codigocentral in (select codigocentral from MarcaCliente where codmarca=" & codmarca & " and activo=1)"
		end if												
		if codterritorio<>"" then
		...
		if fechapromesaini<>"" and fechapromesafin<>"" then
			filtrobuscador = " where A.fechapromesa between '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
		end if												
		if fechapromesaini<>"" and fechapromesafin="" then
			filtrobuscador = " where A.fechapromesa>='" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "'"
		end if														
		if fechapromesaini="" and fechapromesafin<>"" then
			filtrobuscador = " where A.fechapromesa<='" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
		end if																
		if codgestion<>"" then
			if codgestion<>"*" and codgestion<>"**" then
			filtrobuscador = filtrobuscador & " and A.codgestion='" & codgestion & "'"
			end if
			if codgestion="*" then
			filtrobuscador = filtrobuscador & " and A.codgestion is null"
			end if
			if codgestion="**" then
			filtrobuscador = filtrobuscador & " and A.codgestion is not null"
			end if
		end if																	
		...
		''sql="select count(*) from " & vistabusqueda & " A " & filtrobuscador
		''response.write sql
		''consultar sql,RS	
        ...
        if obtener("nuevabusqueda")<>"" or IsNull(RS.Fields(0)) then
            sql="EXEC SP_VERIMPAGADO_PRUEBA '" & vistabusqueda & "','" & session("codusuariodif") & "'," & cantidadxpagina & "," & pag & ",'" & mid(fechaasigini,7,4) & mid(fechaasigini,4,2) & mid(fechaasigini,1,2) & "','" & mid(fechaasigfin,7,4) & mid(fechaasigfin,4,2) & mid(fechaasigfin,1,2) & "'," & xcontrato & "," & xcodigocentral & "," & xcodtipodocumento & "," & xnumdocumento & "," & xdiaatrasoini & "," & xdiaatrasofin & "," & xcodproducto & "," & xcodagencia & "," & xcodmarca & "," & xcodterritorio & "," & xcodoficina & "," & xfechapromesaini & "," & xfechapromesafin & "," & xcodgestion & "," & xcodgestor
            conn.execute sql
            ''response.write sql
		    ''consultar sql,RS		
		    ''RS.close
		...
		sql="select * from ##User" & session("codusuariodif") & " where Nro between " & cantidadxpagina*(pag -1) + 1 & " and " & cantidadxpagina*pag & " order by Nro"
		''response.write sql
		consultar sql,RS	
		if not RS.EOF then
		contadortotal=RS.Fields("totalreg")
		end if
		...
		'''nuevo esquema
		if pag>1 then		
            tablatemporal="#Temp" & session("codusuario") & replace(Date(),"/","") & "h" & Hour(Now()) & "m" & Minute(Now()) & "s" & Second(Now())
        
            sql1="select * into " & tablatemporal & " from " & vistabusqueda & " A " & filtrobuscador
            ''sql1="select A.codigocentral,A.contrato,A.fechadatos,A.codagencia,A.nombre,A.telefono,A.tipofono1,A.prefijo1,A.fono1,A.extension1,A.CodProducto,A.CodSubProducto,A.SubProducto,A.Producto,A.DiasVencimiento,A.Divisa,A.SaldoHoy,A.Agencia,A.FechaGestion,RG.fhgestionado,RG.CodGestion,RG.Gestion,RG.CodTipoContacto,RG.TipoContacto,RG.FechaPromesa,RG.Divisa1 as DivisaPromesa1,RG.Importe1,RG.Divisa2 as DivisaPromesa2,RG.Importe2,A.tipodocumento,A.numdocumento,A.CodTerritorio,A.CodOficina,A.Nro into " & tablatemporal & " from " & vistabusqueda & " A left outer join VISTAMEJORGESTION as RG on A.codigocentral=RG.codigocentral and A.fechadatos=RG.fechadatos and A.contrato=RG.contrato and A.codagencia=RG.codagencia " & filtrobuscador
            
        
	        sql2="select top " & cantidadxpagina & " * " & _
		        "from " & tablatemporal & " A where A.nro not in (select top " & topnovisible & " A.nro from " & tablatemporal & " A order by A.nro) order by A.nro"
		    
		    sql3="Drop Table " & tablatemporal
		   ...
		   '' conn.execute sql1
		    ...
		    sql="select top " & cantidadxpagina & " * " & _
			    "from " & vistabusqueda & " A " & filtrobuscador & " order by A.nro"
			    
            ''sql="select top " & cantidadxpagina & " A.codigocentral,A.contrato,A.fechadatos,A.codagencia,A.nombre,A.telefono,A.tipofono1,A.prefijo1,A.fono1,A.extension1,A.CodProducto,A.CodSubProducto,A.SubProducto,A.Producto,A.DiasVencimiento,A.Divisa,A.SaldoHoy,A.Agencia,A.FechaGestion,RG.fhgestionado,RG.CodGestion,RG.Gestion,RG.CodTipoContacto,RG.TipoContacto,RG.FechaPromesa,RG.Divisa1 as DivisaPromesa1,RG.Importe1,RG.Divisa2 as DivisaPromesa2,RG.Importe2,A.tipodocumento,A.numdocumento,A.CodTerritorio,A.CodOficina,A.Nro " & _
			  ''  "from " & vistabusqueda & " A left outer join VISTAMEJORGESTION as RG on A.codigocentral=RG.codigocentral and A.fechadatos=RG.fechadatos and A.contrato=RG.contrato and A.codagencia=RG.codagencia " & filtrobuscador & " order by A.nro"			    
		end if			
        ...
        ''sql="EXEC SP_VERIMPAGADO_PRUEBA '" & vistabusqueda & "'," & cantidadxpagina & "," & pag & ",'" & mid(fechaasigini,7,4) & mid(fechaasigini,4,2) & mid(fechaasigini,1,2) & "','" & mid(fechaasigfin,7,4) & mid(fechaasigfin,4,2) & mid(fechaasigfin,1,2) & "',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL"
        ''response.write sql
		''consultar sql,RS
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				...
				datos[tabla][<%=contador%>][5]='<%=RS.Fields("subproducto")%>';
