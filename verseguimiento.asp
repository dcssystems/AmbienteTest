...
if session("codusuario")<>"" then
	...
	if permisofacultad("admimpagado.asp") then
		...
		sql="select A.codrespgestion,D.descripcion AS tipoaccion,C.descripcion AS tipocontacto,(CASE WHEN ltrim(rtrim(A.fono))<>'' THEN rtrim(ltrim(A.tipofono)) + '-' + rtrim(ltrim(A.prefijo)) + '-' + rtrim(ltrim(A.fono)) + (CASE WHEN LEN(RTRIM(ltrim(A.extension)))> 0 and A.extension<>'0000' THEN '-'+ RTRIM(LTRIM(A.extension)) ELSE '' END) ELSE A.direccion + ' - ' + UB.distrito + ' - ' + UB.provincia + ' - ' + UB.departamento END) AS dirtef,B.descripcion AS respgestion,A.comentario as comentario,A.ficherogestion AS ficherogestion, A.fechapromesa, A.importe1, A.importe2, A.divisa1,A.divisa2 from RespuestaGestion A inner join Gestion B on A.codgestion=B.codgestion inner join TipoContacto C on B.codtipocontacto=C.codtipocontacto inner join TipoAccion D on B.codtipoaccion=D.codtipoaccion LEFT OUTER JOIN Ubigeo AS UB ON UB.coddpto = A.coddpto AND UB.codprov = A.codprov AND UB.coddist = A.coddist where A.codrespgestion=" & codrespgestion
		consultar sql,RS
		if not RS.EOF then
		    ...
		    fechapromesa=RS.Fields("fechapromesa")
		    divisa1=RS.Fields("divisa1")
		    divisa2=RS.Fields("divisa2")
		    importe1=RS.Fields("importe1")
		    importe2=RS.Fields("importe2")
		end if
		RS.close
		%>
				    ...
				    <table width=100% height=80% border=0 cellspacing=0 cellpadding=0>
			            ...
			            <tr>
				           ...
				           <td bgcolor="#f5f5f5"><font face=Arial size=2 color=#483d8b><b><%=respgestion%></b></font></td> 
				            ...
				            <% if not IsNull(fechapromesa) then %>
							   
    							
	                            <tr>
		                            <td width=22%><font face=Arial size=1.5 color=#483d8b>&nbsp;&nbsp;&nbsp;&nbsp;Fecha de promesa:</font></td>
                                    <td><font face=Arial size=1.5 color=#483d8b><b><%=fechapromesa%></b></font></td>						
	                            </tr>
	                            <% if not IsNull(importe1) then %>
	                                <tr>
		                                <td width=22%><font face=Arial size=1.5 color=#483d8b>&nbsp;&nbsp;&nbsp;&nbsp;Monto a pagar:</font></td>
                                        <td><font face=Arial size=1.5 color=#483d8b><b><%=divisa1%></b>&nbsp;<b><%=importe1%></b></font></td>		
                                        			
	                                </tr>
	                            <% end if%>
	                            
	                            <% if not IsNull(importe2) then %>
	                                <tr>
		                                <td width=22%><font face=Arial size=1.5 color=#483d8b>&nbsp;&nbsp;&nbsp;&nbsp;Monto a pagar:</font></td>
                                        <td><font face=Arial size=1.5 color=#483d8b><b><%=divisa2%></b>&nbsp;<b><%=importe2%></b></font></td>		 				
	                                </tr>
	                            <% end if%>
	                            
	                            <tr>
				                    <td bgcolor="#f5f5f5"><font face=Arial size=2 color=#483d8b>&nbsp;Comentario:</font></td>
				                    <td bgcolor="#f5f5f5"><font face=Arial size=2 color=#483d8b><b><%=comentario%></b></font></td>
			            ...
			            <%else%>
			                    <tr>
				                    ...
				                    <td><font face=Arial size=2 color=#483d8b><b><%=replace(replace(comentario,chr(10),"<BR>"),chr(13),"<BR>")%></b></font></td>
