      ...
      function toggleCodes(on) {
		...
		<script language="javascript">
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Cod','Gestor','Acción','Fecha gestión','Telefono','T.Contacto','Respuesta','Comentario','Editar');
		...
		<%
			filtrobuscador = "where a.IDCampañaPersona = " & datapersona
		if buscador<>"" then
			filtrobuscador = filtrobuscador & " and (a.Comentario like '%" & buscador & "%' or d.Numero like '%" & buscador & "%' )"
		end if
		
		contadortotal=0
		sql="select count(*) from Campaña_Persona_Accion a " & chr(10) & _
				...
				"left outer join Campaña_Persona_Telefono d on a.IDCampañaPersonaTelefono = d.IDCampañaPersonaTelefono" & chr(10) & _
		...
		if pag>1 then					
			sql="select top " & cantidadxpagina &  "a.IDCampañaPersonaAccion as Cod, " & chr(10) & _
				...
				"from Campaña_Persona_Accion a " & chr(10) & _
				"left outer join Usuario b on a.UsuarioEjecutor = b.CodUsuario " & chr(10) & _
				"inner join TipoAccion c on a.IDTipoAccion = C.IDTipoAccion " & chr(10) & _
				"left outer join Campaña_Persona_Telefono d on a.IDCampañaPersonaTelefono = d.IDCampañaPersonaTelefono" & chr(10) & _
				"left outer join Gestion e on e.idGestion = a.IDGestion " & chr(10) & _
				"inner join TipoContacto f on a.IDTipoContacto = f.IDTipoContacto  where " & filtrobuscador1 & " a.IDCampañaPersonaAccion  not in (select top " & topnovisible & " a.IDCampañaPersonaAccion  " & chr(10) & _
							"from Campaña_Persona_Accion a " & chr(10) & _
							"left outer join Usuario b on a.UsuarioEjecutor = b.CodUsuario " & chr(10) & _
							"inner join TipoAccion c on a.IDTipoAccion = C.IDTipoAccion " & chr(10) & _
							"left outer join Campaña_Persona_Telefono d on a.IDCampañaPersonaTelefono = d.IDCampañaPersonaTelefono" & chr(10) & _
							"left outer join Gestion e on e.idGestion = a.IDGestion " & chr(10) & _
							"inner join TipoContacto f on a.IDTipoContacto = f.IDTipoContacto  " & filtrobuscador & " order by a.IDCampañaPersonaAccion desc) order by a.IDCampañaPersonaAccion desc" 
		else
		sql="select top " & cantidadxpagina &  "a.IDCampañaPersonaAccion as Cod, " & chr(10) & _
											   "b.Usuario as Gestor, " & chr(10) & _
											   "c.Descripcion as Accion, " & chr(10) & _
			...
			"from Campaña_Persona_Accion a " & chr(10) & _
							...
							"left outer join Gestion e on e.idGestion = a.IDGestion " & chr(10) & _
							 ...
							 filtrobuscador & " order by a.IDCampañaPersonaAccion desc" 
		end if
		'response.write sql
		consultar sql,RS
		contador=0
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.Perfil','Descripción','Orden'"
