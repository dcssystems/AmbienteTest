		...
		For Each File In Uploader.Files.Items		
			...
			if bolUpload = true then
				if cuentaarch=0 then
					File.FileName="ot" & Uploader.Form("numdocumento") & "-" & Day(Date()) & Month(Date()) & Year(Date()) & "OD." & GetFileExtension(File.FileName)
					...
					File.FileName="ot" & Uploader.Form("numdocumento") & "-" & Day(Date()) & Month(Date()) & Year(Date()) & "OI." & GetFileExtension(File.FileName)
					File.SaveToDisk strFolder ' Save the file			
					''strMessage =  "File Uploaded: " & File.FileName
					strMessage1 = File.FileName
					'strMessage = strMessage & "Size: " & File.FileSize & " bytes<br>"
					'strMessage = strMessage & "Type: " & File.ContentType & "<br><br>"							
				end if
			end if
...
Response.Redirect "nuevaotoscopia.asp?agregardato=" & Uploader.Form("agregardato") & "&idotoscopia=" & Uploader.Form("idotoscopia") & "&iddocidentidad=" & Uploader.Form("iddocidentidad") & "&numdocumento=" & Uploader.Form("numdocumento") & "&positiva=" & Uploader.Form("positiva") & "&odcae=" & Uploader.Form("odcae") & "&oicae=" & Uploader.Form("oicae") & "&odmt=" & Uploader.Form("odmt") & "&oimt=" & Uploader.Form("oimt") & "&imagen=" & strMessage & "&imagen1=" & strMessage1
