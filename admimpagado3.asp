...
<%Response.Flush()%>
	...
	<script language=javascript>
        ...
        if obtener("nuevabusqueda")<>"" or IsNull(RS.Fields(0)) then
            ...
            sql1="select DD.codigocentral, DD.contrato, DD.fechadatos, DD.codagencia, DD.nombre,DD.codterritorio,TE.descripcion AS Territorio,DD.codoficina,OFI.descripcion AS oficina,DD.telefono, DD.tipofono1, DD.prefijo1, " &_
                "DD.fono1, DD.extension1,  DD.CodProducto,DD.CodSubProducto,SP.descripcion as SubProducto,FP.descripcion as Producto,DD.Divisa, DD.SaldoHoy, " &_
                "CASE WHEN DD.divisa <>'PEN' THEN DD.SaldoHoy*TCA.fixing ELSE DD.SaldoHoy END AS SaldoHoySoles,CASE WHEN DD.saldohoy*TCA.fixing <= 10 THEN 'RI1 [<10]' WHEN DD.saldohoy * TCA.fixing <= 100 THEN 'RI2 <10 - 100]' WHEN DD.saldohoy * TCA.fixing <= 200 THEN 'RI3 <100 - 200]' " &_
                "WHEN DD.saldohoy * TCA.fixing <= 300 THEN 'RI4 <200 - 300]' WHEN DD.saldohoy * TCA.fixing <= 500 THEN 'RI5 <300 - 500]' WHEN DD.saldohoy * TCA.fixing <= 1000 THEN " &_
                "'RI6 <500 - 1000]' WHEN DD.saldohoy * TCA.fixing <= 1500 THEN 'RI7 <1000 - 1500]' WHEN DD.saldohoy * TCA.fixing <= 2000 THEN 'RI8 <1500 - 2000]' WHEN DD.saldohoy " &_
                "* TCA.fixing <= 2500 THEN 'RI9 <2000 - 2500]' WHEN DD.saldohoy * TCA.fixing <= 3000 THEN 'RI10 <2500 - 3000]' WHEN DD.saldohoy * TCA.fixing <= 3500 THEN 'RI11 <3000 - 3500]' " &_
                "WHEN DD.saldohoy * TCA.fixing <= 4000 THEN 'RI12 <3500 - 4000]' WHEN DD.saldohoy * TCA.fixing <= 5000 THEN 'RI13 <4000 - 5000]' WHEN DD.saldohoy * TCA.fixing <= " &_
                "6000 THEN 'RI14 <5000 - 6000]' WHEN DD.saldohoy * TCA.fixing <= 7000 THEN 'RI15 <6000 - 7000]' WHEN DD.saldohoy * TCA.fixing <= 8000 THEN 'RI16 <7000 - 8000]' WHEN " &_
                "DD.saldohoy * TCA.fixing <= 9000 THEN 'RI17 <8000 - 9000]' WHEN DD.saldohoy * TCA.fixing <= 10000 THEN 'RI18 <9000 - 10000]' WHEN DD.saldohoy * TCA.fixing <= 11000 " &_
                "THEN 'RI19 <10000 - 11000]' WHEN DD.saldohoy * TCA.fixing <= 12000 THEN 'RI20 <11000 - 12000]' WHEN DD.saldohoy * TCA.fixing <= 13000 THEN 'RI21 <12000 - 13000]' " &_
                "WHEN DD.saldohoy * TCA.fixing > 13000 THEN 'RI22 [>13000]' END AS RangoImpago,DD.DiasVencimiento, CASE WHEN DD.DiasVencimiento <= 30 THEN 'T1 [1-30]' " &_
                "WHEN DD.DiasVencimiento <= 60 THEN 'T2 [31-60]' WHEN DD.DiasVencimiento <= 90 THEN 'T3 [61-90]' WHEN DD.DiasVencimiento <= 120 THEN 'T4 [91-120]' END AS TramoReal, " &_
                "MA.descripcion AS Marca,AG.razonsocial as Agencia, DD.FechaGestion,  RG.fhgestionado, GS.codgestion,GS.CodGestion + '-' + GS.descripcion as Gestion, " &_
                "IsNull(GS.codtipocontacto,'') as CodTipoContacto, IsNull(TC.descripcion,'') as TipoContacto, IsNull(convert(varchar,RG.FechaPromesa,103),'') + ' ' + IsNull(convert(varchar,RG.FechaPromesa,108),'') AS fechapromesa, " &_
                ...
                "RG.Divisa1 AS DivisaPromesa1, RG.Importe1, RG.Divisa2 AS DivisaPromesa2, RG.Importe2,DD.tipodocumento, DD.numdocumento, DD.Nro into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD " &_
                "inner join Agencia AG on DD.codagencia=AG.codagencia left outer join Producto PR on DD.codproducto=PR.codproducto left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto " &_
                "left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion " &_
                "left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto left outer join Oficina OFI " &_
                "ON DD.codoficina=OFI.codoficina left outer join Territorio TE ON DD.CodTerritorio=TE.codterritorio left outer join TipoCambio TCA ON DD.fechadatos=TCA.fechadatos AND DD.Divisa=TCA.divisa and TCA.tipo='S' " &_
                "left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 left outer join Marca MA ON MC.codmarca=MA.codmarca " &_
                "left outer join Ubigeo UBI on RG.coddpto=UBI.coddpto and RG.coddist=UBI.coddist and RG.codprov=UBI.codprov " & filtrobuscador
...
<table border=0 cellspacing=0 cellpadding=2 width=100%>
  ...
  <tr bgcolor="#E9F8FE">
    ...
    <select name="codfamproducto" style="font-size: x-small; width: 250px;">
				...
				Do While Not  RS.EOF
				%>
					<option value="<%=RS.Fields("codfamproducto")%>" <% if codfamproducto<>"" then%><% if RS.fields("codfamproducto")=int(codfamproducto) then%> selected<%end if%><%end if%>><%=RS.Fields("codfamproducto") & " - " & RS.Fields("Descripcion")%></option>
				<%
				RS.MoveNext
				loop
				RS.Close
