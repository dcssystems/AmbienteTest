...
if session("codusuario")<>"" then
	...
	if permisofacultad("cargatelefono.asp") then
					...
					For Each File In Uploader.Files.Items		
						...
						if bolUpload = true then
							if cuentaarch=0 then
								...
								''mid(Uploader.Form("fecha"),7,4) & mid(Uploader.Form("fecha"),4,2) & mid(Uploader.Form("fecha"),1,2)
								''mid(Uploader.Form("fecha"),7,4) & mid(Uploader.Form("fecha"),4,2) & mid(Uploader.Form("fecha"),1,2)
								''mid(Uploader.Form("fecha"),7,4) & mid(Uploader.Form("fecha"),4,2) & mid(Uploader.Form("fecha"),1,2)
								sql="INSERT INTO CargaArchivo(proceso,rutaarchivo,mailinforme,usuarioregistra,fecharegistra,codestadocarga) VALUES ('telefonos','" & RutaFisicaUpload & "\telefonos','" & correo & "'," & session("codusuario") & ",getdate(),1)"
								conn.execute sql		
								
								''capturamos el maximo CargaArchivo para el FileName que sera "telefonos(nro).txt"
								...
								File.FileName="telefonos" & codcarga & "." & GetFileExtension(File.FileName)
			...
			''Response.Redirect "cargatelefono.asp?agregardato=" & Uploader.Form("agregardato") & "&flaginactivar=" & Uploader.Form("flaginactivar") & "&archivotxt=" & strMessage
			%>
				<script language=javascript>
					window.open("cargatelefono.asp?agregardato=<%=Uploader.Form("agregardato")%>&flaginactivar=<%=Uploader.Form("flaginactivar")%>&archivotxt=<%=strMessage%>","_self");
				</script>
			<%
