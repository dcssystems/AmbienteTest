...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("admgrupoagencia.asp") then
	...
	%>
		...
		{
			ventanagrupoagencia=global_popup_IWTSystem(ventanagrupoagencia,"nuevogrupoagencia.asp?vistapadre=" + window.name + "&paginapadre=admgrupoagencia.asp&codgrupoagencia=" + codigo,"NewUser","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanagrupoagencia=global_popup_IWTSystem(ventanagrupoagencia,"nuevogrupoagencia.asp?vistapadre=" + window.name + "&paginapadre=admgrupoagencia.asp","Newgrupoagencia","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    columnavisible[tabla] = new Array( false, true, true);
		    ...
		    objetofomulario[tabla] = new Array();
				...
				objetofomulario[tabla][1]='<input type=hidden name=codgrupoagencia-id- value=-c0->' + '<a href="javascript:modificar(-id-);">-valor-</a>';
		...
		contadortotal=0
		sql="select count(*) from grupoagencia  " & filtrobuscador 
		consultar sql,RS	
		...
		sql="select top " & cantidadxpagina & " * " & _
			"from grupoagencia where " & filtrobuscador1 & " codgrupoagencia not in (select top " & topnovisible & " codgrupoagencia from grupoagencia " & filtrobuscador & " order by codgrupoagencia) order by codgrupoagencia" 
		else
		sql="select top " & cantidadxpagina & " * " & _
			"from grupoagencia " & filtrobuscador & " order by codgrupoagencia" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codgrupoagencia" & RS.Fields("codgrupoagencia"))<>"" then
					
					if obtener("descripcion" & RS.Fields("codgrupoagencia"))<>"" then
						descripcion=obtener("descripcion" & RS.Fields("codgrupoagencia"))
					else
						descripcion=""
					end if
		
					''if obtener("descripcion" & RS.Fields("codgrupoagencia")) <> rs.Fields("descripcion") then
							''sql="update grupoagencia set descripcion = '"& descripcion &"' ,usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codgrupoagencia= " & rs.Fields("codgrupoagencia") 
							''response.write sql
							''conn.Execute sql
					''end if
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]=<%=RS.Fields("codgrupoagencia")%>;
			    datos[tabla][<%=contador%>][1]='<%if obtener("actualizarlista")<>"" and obtener("codgrupoagencia" & RS.Fields("codgrupoagencia"))<>"" then%><%=descripcion%><%else%><%=rs.Fields("descripcion")%><%end if%>';
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Grupo Agencia (0) - No hay registros.</b></font>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Grupo Agencia (<%=contadortotal%>)&nbsp;&nbsp;<!--<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;--><a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=middle width=200><font size=2 face=Arial color=#00529B>Buscar:&nbsp;<input name="buscador" value="<%=buscador%>" size=20 onkeypress="if(window.event.keyCode==13) buscar();"></font></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Descripción'"
					...
					consulta_exp="select descripcion " & _
								 "from CobranzaCM.dbo.grupoagencia " & filtrobuscador & " order by codgrupoagencia" 
