...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("repidasegmentopart.asp") then
		...
		<!--cargando--><%Response.Flush()%><img src=imagenes/loading.gif border=0 id="imgloading" name="imgloading" style="margin-left: 50px;margin-top:50px;"><%Response.Flush()%>
			...
			<form name=formula method=post>
					...
					<tr>
						...
						<font size=4 color=#483d8b face=Arial><b>Informe Diario de Mora (IDA - BC) al <%=fechacierre%> - PARTICULARES</b></font></td>
					</tr>
					</table>
					...
					<tr>
						...
						<td bgcolor="#007DC5" align="center" width="5%">
							<font size=1 color=#FFFFFF face=Arial><b>PART</b></font>
						</td>
						<!--<td bgcolor="#007DC5" align="center" width="5%">
							<font size=1 color=#FFFFFF face=Arial><b>PYME</b></font>
						</td>-->
					</tr>		
					</table>
					</div>
					...
					<%
	                    ...
	                    sql="select * into #CasosJudDia from FB.CentroCobranzas.dbo.PD_Casos_Jud where SEGMENTO_RIESGO in ('PARTICULARES','PYME') and FECHADATOS='" & mid(fechacierre,7,4) & mid(fechacierre,4,2) & mid(fechacierre,1,2) & "'"
	                    conn.execute sql
					    ...
					    sql="SELECT " &_
			                    ...
			                    "    D.ESPECIALISTA_JUD_PART," &_
			                    "    D.ESPECIALISTA_JUD_PYME," &_
				                ...
				                "        AND ltrim(rtrim(A.segmento_riesgo)) in ('PARTICULARES', 'PYME') " &_
				                "        AND ltrim(rtrim(A.ENTIDAD))			in ('BC','35') " &_
		                        "GROUP BY	A.cod_territorio, " &_
					            "            A.TERRITORIO_HOMOLOGADO, " &_
                                "    D.ESPECIALISTA_JUD_PART," &_
			                    "    D.ESPECIALISTA_JUD_PYME, ltrim(rtrim(A.SEGMENTO_RIESGO)) "
		                ...
		                sql="SELECT	 " &_
				                ...
				                "        AND RTRIM(LTRIM(B.SEGMENTO_RIESGO))	in ('PARTICULARES', 'PYME')  " &_
				                "        AND ltrim(rtrim(A.entidad))			in ('BC','35')	 " &_
		                ...
		                sql="SELECT	" &_
			                "B.cod_territorio, " &_
			                "B.TERRITORIO_HOMOLOGADO, " &_
			                "SUM(CASE WHEN A.FECHA_SALIDAS>'" & mid(fechacierremesant,7,4) & mid(fechacierremesant,4,2) & mid(fechacierremesant,1,2) & "' THEN soles + dolares*tipocambio ELSE 0 END) as Salidas, " &_
			                "SUM(soles + dolares*tipocambio) as SalidasAAAA, " &_
                        ...
                        "    D.ESPECIALISTA_JUD_PART," &_
		                "    D.ESPECIALISTA_JUD_PYME"			        				        
				        
				        conn.execute sql		
				        
					    
                        sql="select codterritorio,territorio,especialista,SUM(jud) as StockCierre,IsNull((select sum(ingresos) from #INGRESOS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as INGRESOS,IsNull((select sum(salidas) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as SALIDAS,IsNull((select sum(salidasconventa) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as SALIDASCONVENTA,IsNull((select sum(REF) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as REF,IsNull((select sum(TRA) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as TRA,IsNull((select sum(EJEC) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as EJEC,IsNull((select sum(EFE) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as EFE,IsNull((select sum(OTRO) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as OTRO,0 as PYME,IsNull((select sum(Salidas) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as PART,IsNull((select sum(SalidasAAAA) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as SalidasAAAA,IsNull((select sum(SalidasAAAAConVenta) from #SALIDAS where cod_territorio=A.codterritorio and SEGMENTO_RIESGO=A.segmento_riesgo and ESPECIALISTA_JUD_PART=A.Especialista),0) as SalidasAAAAConVenta, " &_
                        ...
                        <tr>
	                        <td bgcolor="<%=bgcolor%>">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=RS.Fields("Especialista") & " (" & RS.Fields("TERRITORIO") & ")"%></font></a>
	                        </td>
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("StockCierre")/1000,0)%></font></a>
	                        </td>						
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("Ingresos")/1000,0)%></font></a>
	                        </td>	
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("MetaMes")/1000,0)%></font></a>
	                        </td>	
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("MetaMes")*12/1000,0)%></font></a>
	                        </td>	
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("SalidasAAAAConVenta")/1000,0)%></font></a>
	                        </td>	                        
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("SalidasAAAA")/1000,0)%></font></a>
	                        </td>	                        						                        	                        
						    <td bgcolor="<%=bgcolor%>" align="right" width="5%">
							    <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0),2)%>%</font></a>
						    </td>	
						    <!--<td bgcolor="<%=bgcolor%>" align="center" width="15"><a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><img src="imagenes/<%=iif(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0)>=90,"sem_verde",iif(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0)>=80,"sem_amarillo","sem_rojo"))%>.png" title="<%=iif(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0)>=90,"[90%-100%]",iif(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0)>=80,"[80%-90%]","[0%-80&gt;"))%>" alt="<%=iif(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0)>=90,"[90%-100%]",iif(iif(RS.Fields("MetaMes")>0,iif(RS.Fields("SalidasAAAA")>0,RS.Fields("SalidasAAAA")*100/iif(RS.Fields("MetaMes")>0,RS.Fields("MetaMes")*12,1),0),0)>=80,"[80%-90%]","[0%-80&gt;"))%>" width="12" border=0></a></td>-->
						    <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("SalidasConVenta")/1000,0)%></font></a>
	                        </td>	
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("Salidas")/1000,0)%></font></a>
	                        </td>	                        						                        
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
	                        ...
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("REF")/1000,0)%></font></a>
	                        </td>	 
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("TRA")/1000,0)%></font></a>
	                        </td>	 
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("EJEC")/1000,0)%></font></a>
	                        </td>	 
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("EFE")/1000,0)%></font></a>
	                        </td>	 	                        	                        	                        	                        
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("OTRO")/1000,0)%></font></a>
	                        </td>	 	                        	 
						    <td bgcolor="#FFFFFF" align="center" width="1%">
							    <font size=1 color=#FFFFFF face=Arial><b>&nbsp;</b></font>
						    </td>	                                               	                        	                        	                        
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=particulares&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("PART")/1000,0)%></font></a>
	                        </td>	
	                        <!--<td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidasegmentopart.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=<%=RS.Fields("CODTERRITORIO")%>&plaza=&segmento=pyme&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(RS.Fields("PYME")/1000,0)%></font></a>
	                        </td>-->		                        	                        
                        </tr>	                        
                        <%
                        ...
                        SUMA_PART=SUMA_PART + RS.Fields("PART")/1000
                        SUMA_PYME=SUMA_PYME + RS.Fields("PYME")/1000
                        ...
                        <tr>
						    ...
						    <td bgcolor="#007DC5" align="right" width="5%">
							    <font size=1 color=#FFFFFF face=Arial><b><%=FormatNumber(SUMA_PART,0)%></b></font>
						    </td>
						    <!--<td bgcolor="#007DC5" align="right" width="5%">
							    <font size=1 color=#FFFFFF face=Arial><b><%=FormatNumber(SUMA_PYME,0)%></b></font>
						    </td>												-->
					    </tr>					    
					</table>
