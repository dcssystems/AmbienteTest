...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("dcs_admmicampaña.asp") then
    ...
    <script>
      function toggleCodes(on) {
		...
		{
			window.parent.agregarcol('' + codigo,nombrecamp.substring(0,12),'dcs_vercampañaoperador.asp?idcampana=' + codigo);
		}		
		...
		<script language="javascript">
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('IDCampaña', 'Cliente', 'TipoCampaña', 'Descripcion', 'FechaInicio', 'FechaFin', 'FlagHistorico', 'Estado','Detalle');
		    ...
		    objetofomulario[tabla] = new Array();
				...
				objetofomulario[tabla][8]='<a href="javascript:vercamp(-id-,\'-c2-\');"><i class="demo-icon2 icon-phone-circled">&#xe822;</i> </a>';
		...
		contadortotal=0
		sql="SELECT COUNT(distinct a.IDCampaña) " & _
			"FROM campaña a inner join Cliente b  on a.IDCliente = b.IDCliente " & _
			"INNER JOIN TipoCampaña c  " & _
			"on a.IDTipoCampaña = c.IDTipoCampaña inner join Campaña_Persona e on a.IDCampaña = e.IDCampaña " & filtrobuscador 
		...
		if pag>1 then					
		sql="SELECT distinct TOP " & cantidadxpagina & " a.IDCampaña, b.RazonSocial as Cliente, c.Descripcion as TipoCampaña , a.Descripcion, a.FechaInicio, a.FechaFin, a.FlagHistorico, d.Descripcion AS Estado from campaña a inner join Cliente b  on a.IDCliente = b.IDCliente inner join TipoCampaña c on a.IDTipoCampaña = c.IDTipoCampaña inner join Estado d on d.CodEstado = a.Estado and d.Tabla = 'Campaña' WHERE " & filtrobuscador1 & " a.IDCampaña NOT IN (SELECT TOP " & topnovisible & " campaña a inner join Cliente b  on a.IDCliente = b.IDCliente inner join TipoCampaña c on a.IDTipoCampaña = c.IDTipoCampaña inner join Estado d on d.CodEstado = a.Estado and d.Tabla = 'Campaña' inner join Campaña_Persona e on a.IDCampaña = e.IDCampaña " & filtrobuscador & " ORDER BY a.IDCampaña) ORDER BY a.IDCampaña" 
		else
		sql="SELECT distinct TOP " & cantidadxpagina & " a.IDCampaña, b.RazonSocial as Cliente, c.Descripcion as TipoCampaña , a.Descripcion, a.FechaInicio, a.FechaFin, a.FlagHistorico, d.Descripcion AS Estado from campaña a inner join Cliente b  on a.IDCliente = b.IDCliente inner join TipoCampaña c on a.IDTipoCampaña = c.IDTipoCampaña inner join Estado d on d.CodEstado = a.Estado and d.Tabla = 'Campaña' inner join Campaña_Persona e on a.IDCampaña = e.IDCampaña " & filtrobuscador & "  ORDER BY a.IDCampaña" 
		end if
		'response.write sql
		consultar sql,RS
		contador=0
