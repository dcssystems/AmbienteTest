...
<table border=0 cellspacing=0 cellpadding=2 width=100%>
  ...
  <td colspan="17">
		...
		''Si se pide exportar a excel
				if expimp="1" then
                            ''**********NUEVO PROCESO DE EXPORTACION*****************
		                    sql="select OBJECT_ID('CobranzaCM.dbo.UserExpTmp" & session("codusuario") & "')"
	                        consultar sql,RS
                            if not IsNull(RS.Fields(0)) then
                                sql="drop table UserExpTmp" & session("codusuario")
                                conn.execute sql
                            end if
                            RS.Close                    
                            
                            sql="select Nro into UserExpTmp" & session("codusuario") & " from ##User" & session("codusuariodif")
                            conn.execute sql	

                            sql="Delete ExpPendientes where codusuario=" & session("codusuario")
                            conn.execute sql     
                            
                            sql="Insert Into ExpPendientes (CodUsuario) values (" & session("codusuario") & ")"
                            conn.execute sql                    
				        %>
					        <script language="javascript">
