...
if session("codusuario")<>"" then
	...
	codmarca=obtener("codmarca")
		...
		if obtener("activo")<>"" then activo="1" else activo="0" end if	
			...
			if codmarca<>"" then
			sql="select count(*) from marca where descripcion='" & descripcion & "' and codmarca<>" & codmarca 
			else
			sql="select count(*) from marca where descripcion='" & descripcion & "'"
			end if
			consultar sql,RS
			existemarca=RS.Fields(0)
			RS.Close			
			if existemarca=0 then			
				if obtener("agregardato")="1" then		
					sql="insert into marca (descripcion,activo,usuarioregistra,fecharegistra) values ('" & descripcion & "'," & activo & "," & session("codusuario") & ",getdate())"
				else
					sql="update marca set descripcion='" & descripcion & "',activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codmarca=" & codmarca
				end if
				''Response.Write sql
				conn.execute sql
				...
				<script language=javascript>
					...
					<%end if%>				
					<%if obtener("paginapadre")="admmarcas.asp" then%>window.open("<%=obtener("paginapadre")%>","<%=obtener("vistapadre")%>");<%end if%>
					window.close();
				</script>			
				<%
			else
			...
			if codmarca<>"" then
					sql="select A.*,B.nombres as Nombreusureg, B.apepaterno as Apepatusureg, B.apematerno as Apematusureg, C.nombres as Nombreusumod,C.apepaterno as Apepatusumod, C.apematerno as Apematusumod from marca A inner join usuario B on B.codusuario=A.usuarioregistra left outer join usuario C on C.codusuario=A.usuariomodifica where a.codmarca = " & codmarca
					consultar sql,RS
		...
		<html>
			<title><%if codmarca="" then%>Nueva <%end if%>Marca</title>
			...
			<body topmargin=0 leftmargin=0 bgcolor="#FFFFFF">
					...
					<form name=formula method=post action="nuevomarcas.asp">
					<tr height=20>	
						<td bgcolor="#F5F5F5" colspan=2>			
							<font size=2 color=#483d8b face=Arial><b>&nbsp;<b><%if codmarca="" then%>Nuevo <%end if%>Marca</b></b></font>
						</td>
					</tr>
					<%if fechaReg<>"" then%>
					<tr height=20>
					...
					<tr>					
						...
						<td bgcolor="#F5F5F5" align=right height=40><%if codmarca="" then%><a href="javascript:agregar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar"></a>&nbsp;<%else%><a href="javascript:modificar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar"></a>&nbsp;<%end if%><a href="javascript:window.close();"><img src="imagenes/salida.gif" border=0 alt="Salir" title="Salir"></a>&nbsp;</td>					
					</tr>
