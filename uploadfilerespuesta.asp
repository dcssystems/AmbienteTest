...
if session("codusuario")<>"" then
	...
	if permisofacultad("cargarespuesta.asp") then
					...
					For Each File In Uploader.Files.Items		
						...
						if bolUpload = true then
							if cuentaarch=0 then
								...
								''Estado Pendiente inserción
								sql="INSERT INTO CargaArchivo(proceso,rutaarchivo,mailinforme,usuarioregistra,fecharegistra,codestadocarga) VALUES ('Respuestas_A" & Uploader.Form("codagencia") & "_F' + convert(varchar,convert(datetime,'" & mid(Uploader.Form("fecha"),7,4) & mid(Uploader.Form("fecha"),4,2) & mid(Uploader.Form("fecha"),1,2) & "'),112),'" & RutaFisicaUpload & "\respuestas','" & correo & "'," & session("codusuario") & ",getdate(),1)"
								conn.execute sql		
								
								''capturamos el maximo CargaArchivo para el FileName que sera "respuestas(nro).txt"
								...
								NombreArchivo="respuestas" & codcarga & "." & GetFileExtension(File.FileName)
			...
			''Response.Redirect "cargarespuesta.asp?agregardato=" & Uploader.Form("agregardato") & "&flaginactivar=" & Uploader.Form("flaginactivar") & "&archivotxt=" & strMessage
			%>
				<script language=javascript>
					window.open("cargarespuesta.asp?agregardato=<%=Uploader.Form("agregardato")%>&flaginactivar=<%=Uploader.Form("flaginactivar")%>&archivotxt=<%=strMessage%>","_self");
				</script>
			<%
