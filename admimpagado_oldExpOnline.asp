...
if session("codusuario")<>"" then
	...
	sql="select max(fechagestion) from BusquedaDistribucion"
	consultar sql,RS	
...
<%Response.Flush()%>
	...
	<script language=javascript>
        ...
        filtroDD = " where fechagestion between convert(datetime,'" & mid(fechaasigini,7,4) & mid(fechaasigini,4,2) & mid(fechaasigini,1,2) & "') and convert(datetime,'" & mid(fechaasigfin ,7,4) & mid(fechaasigfin,4,2) & mid(fechaasigfin,1,2) & "')"
		if contrato<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.contrato='" & contrato & "'"
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " contrato='" & contrato & "'"
		end if
		if codigocentral<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.codigocentral='" & codigocentral & "'"
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " codigocentral='" & codigocentral & "'"
		end if		
		if codtipodocumento<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.tipodocumento='" & codtipodocumento & "'"
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " tipodocumento='" & codtipodocumento & "'"
		end if		
		if numdocumento<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.numdocumento='" & numdocumento & "'"
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " numdocumento='" & numdocumento & "'"
		end if		
		if diaatrasoini<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.diasvencimiento>=" & diaatrasoini
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " diasvencimiento>=" & diaatrasoini
		end if				
		if diaatrasofin<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.diasvencimiento<=" & diaatrasofin
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " diasvencimiento<=" & diaatrasofin
		end if						
		if codagencia<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.codagencia=" & codagencia
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " codagencia=" & codagencia
		end if	
		if codmarca<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.codigocentral in (select codigocentral from MarcaCliente where codmarca=" & codmarca & " and activo=1)"
			''filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " codigocentral in (select codigocentral from MarcaCliente where codmarca=" & codmarca & " and activo=1)"
			filtroMC = " inner join MarcaCliente MC on MC.codigocentral=DD.codigocentral and MC.codmarca=" & codmarca & " and MC.activo=1 "
		end if												
		if codterritorio<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.codterritorio='" & codterritorio & "'"
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " codterritorio='" & codterritorio & "'"
		end if												
		if codoficina<>"" then
			''filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " DD.codoficina='" & codoficina & "'"
			filtroDD = filtroDD & iif(filtroDD=""," where"," and") & " codoficina='" & codoficina & "'"
		end if										
		if codfamproducto<>"" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " and FP.codfamproducto=" & codfamproducto
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " and FP.codfamproducto=" & codfamproducto
			filtroFP = filtroFP & iif(filtroFP=""," where"," and") & " and FP.codfamproducto=" & codfamproducto
		end if								
		if fechapromesaini<>"" and fechapromesafin<>"" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " RG.fechapromesa between '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " RG.fechapromesa between '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
			filtroRG = filtroRG & iif(filtroRG=""," where"," and") & " RG.fechapromesa between '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
		end if												
		if fechapromesaini<>"" and fechapromesafin="" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " RG.fechapromesa>='" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "'"
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " RG.fechapromesa>='" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "'"
			filtroRG = filtroRG & iif(filtroRG=""," where"," and") & " RG.fechapromesa>='" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "' and '" & mid(fechapromesaini,7,4) & mid(fechapromesaini,4,2) & mid(fechapromesaini,1,2) & "'"
		end if														
		if fechapromesaini="" and fechapromesafin<>"" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " RG.fechapromesa<='" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " RG.fechapromesa<='" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
			filtroRG = filtroRG & iif(filtroRG=""," where"," and") & " RG.fechapromesa<='" & mid(fechapromesafin,7,4) & mid(fechapromesafin,4,2) & mid(fechapromesafin,1,2) & "'"
		end if																
		if codgestion<>"" then
			if codgestion<>"*" and codgestion<>"**" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " RG.codgestion='" & codgestion & "'"
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " RG.codgestion='" & codgestion & "'"
			filtroRG = filtroRG & iif(filtroRG=""," where"," and") & " RG.codgestion='" & codgestion & "'"
			end if
			if codgestion="*" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " RG.codgestion is null"
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " RG.codgestion is null"
			filtroRG = filtroRG & iif(filtroRG=""," where"," and") & " RG.codgestion is null"
			end if
			if codgestion="**" then
			filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " RG.codgestion is not null"
			filtroOtros = filtroOtros & iif(filtroOtros=""," where"," and") & " RG.codgestion is not null"
			filtroRG = filtroRG & iif(filtroRG=""," where"," and") & " RG.codgestion is not null"
			end if
		end if																	
		''Preventiva/Impagado				
		''if tipogestion<>"" then
		''	filtrobuscador = filtrobuscador & iif(filtrobuscador=""," where"," and") & " A.tipogestion='" & tipogestion & "'"
		''end if																
        ...
        if obtener("nuevabusqueda")<>"" or IsNull(RS.Fields(0)) then
            ...
            ''sql1="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.codagencia,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,DD.codproducto, DD.CodSubProducto,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,GS.CodTipoContacto,TC.descripcion as TipoContacto,RG.FechaPromesa,RG.Divisa1 AS DivisaPromesa1,RG.Importe1,RG.Divisa2 AS DivisaPromesa2, RG.Importe2,DD.tipodocumento,DD.numdocumento,DD.CodTerritorio,DD.CodOficina, DD.Nro into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join Producto PR on DD.codproducto=PR.codproducto left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto left outer join Oficina OFI ON DD.codoficina=OFI.codoficina left outer join Territorio TE ON DD.CodTerritorio=TE.codterritorio left outer join TipoCambio TCA ON DD.fechadatos=TCA.fechadatos AND DD.Divisa=TCA.divisa and TCA.tipo='S' left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 left outer join Marca MA ON MC.codmarca=MA.codmarca left outer join Ubigeo UBI on RG.coddpto=UBI.coddpto and RG.coddist=UBI.coddist and RG.codprov=UBI.codprov " & filtrobuscador
            ''sql1="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,TC.descripcion as TipoContacto,DD.Nro into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join Producto PR on DD.codproducto=PR.codproducto left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto left outer join Oficina OFI ON DD.codoficina=OFI.codoficina left outer join Territorio TE ON DD.CodTerritorio=TE.codterritorio left outer join TipoCambio TCA ON DD.fechadatos=TCA.fechadatos AND DD.Divisa=TCA.divisa and TCA.tipo='S' left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 left outer join Marca MA ON MC.codmarca=MA.codmarca left outer join Ubigeo UBI on RG.coddpto=UBI.coddpto and RG.coddist=UBI.coddist and RG.codprov=UBI.codprov " & filtrobuscador
            ''sql1="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,TC.descripcion as TipoContacto,DD.Nro,DD.codterritorio,DD.CodOficina,DD.fechaincumplimiento,DD.codestado,DD.coddistrito,DD.codprovincia,RG.coddpto as RGcoddpto,RG.coddist as RGcoddist,RG.codprov as RGcodprov,DD.direccion,RG.direccion as RGdireccion,RG.fechapromesa,RG.tipofono,RG.prefijo,RG.fono,RG.extension into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia /*left outer join Producto PR on DD.codproducto=PR.codproducto*/ left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto /*left outer join Oficina OFI ON DD.codoficina=OFI.codoficina left outer join Territorio TE ON DD.CodTerritorio=TE.codterritorio left outer join TipoCambio TCA ON DD.fechadatos=TCA.fechadatos AND DD.Divisa=TCA.divisa and TCA.tipo='S' left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 left outer join Marca MA ON MC.codmarca=MA.codmarca left outer join Ubigeo UBI on RG.coddpto=UBI.coddpto and RG.coddist=UBI.coddist and RG.codprov=UBI.codprov*/ " & filtrobuscador
            ''sql1="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,TC.descripcion as TipoContacto,DD.Nro,DD.codterritorio,DD.CodOficina,DD.fechaincumplimiento,DD.codestado,DD.coddistrito,DD.codprovincia,RG.coddpto as RGcoddpto,RG.coddist as RGcoddist,RG.codprov as RGcodprov,DD.direccion,RG.direccion as RGdireccion,RG.fechapromesa,RG.tipofono,RG.prefijo,RG.fono,RG.extension into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " & filtrobuscador
            ''sql1="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,TC.descripcion as TipoContacto,DD.Nro into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " & filtrobuscador
            
            ''sql1="select DD.Nro into ##User" & session("codusuariodif") & " from BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " & filtrobuscador
            ''sql1="select DD.Nro into ##User" & session("codusuariodif") & " from (select nro,codagencia,codsubproducto,mejor_codrespgestion from BusquedaDistribucion " & filtroDD & ") DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " & filtroOtros
            sql1="select DD.Nro into ##User" & session("codusuariodif") & " from (select nro,codagencia,codsubproducto,mejor_codrespgestion,codigocentral from BusquedaDistribucion " & filtroDD & ") DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join (select codfamproducto from FamProducto " & filtroFP & ") FP ON SP.codfamproducto=FP.codfamproducto left outer join (select codrespgestion,codgestion from RespuestaGestion " & filtroRG & ") RG on DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " & filtroMC
		...
		'''nuevo esquema - Consulta todos los campos con Temp solo Nro
		if pag>1 then					
		sql="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,TC.descripcion as TipoContacto,DD.Nro from (select * from BusquedaDistribucion where nro in (select top " & cantidadxpagina & " A.Nro from ##User" & session("codusuariodif") & " A where A.nro not in (select top " & topnovisible & " A.nro from ##User" & session("codusuariodif") & " A order by A.nro) order by A.nro)) DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto order by DD.nro" 
		else
		sql="select DD.codigocentral, DD.contrato,DD.fechadatos,DD.nombre,DD.telefono,DD.tipofono1,DD.prefijo1,DD.fono1,DD.extension1,IsNull(SP.descripcion,'') as SubProducto, IsNull(FP.descripcion,'') as Producto, DD.DiasVencimiento,DD.Divisa,DD.SaldoHoy,AG.razonsocial as Agencia,DD.FechaGestion,RG.fhgestionado,RG.CodGestion,GS.descripcion as Gestion,TC.descripcion as TipoContacto,DD.Nro from (select * from BusquedaDistribucion where nro in (select top " & cantidadxpagina & " A.Nro from ##User" & session("codusuariodif") & " A order by A.nro)) DD inner join Agencia AG on DD.codagencia=AG.codagencia left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD.mejor_codrespgestion=RG.codrespgestion left outer join Gestion GS ON RG.codgestion = GS.codgestion left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto order by DD.nro" 
		end if		
		''response.write sql
		consultar sql,RS
  ...
  <td colspan="17">
		...
		''Si se pide exportar a excel
				if expimp="1" then
                    ...
                    sql="select convert(varchar,DD.FechaGestion,103) AS FechaAsig, " &_
                        "char(39) + DD.contrato AS contrato," &_ 
                        "char (39) + DD.codigocentral AS codigocentral, " &_
                        "DD.nombre," &_
                        "char(39) + DD.codterritorio as codterritorio," &_
                        "TE.descripcion AS Territorio," &_
                        "char(39) + DD.codoficina as codoficina," &_
                        ...
                        "CASE WHEN DD.saldohoy*TCA.fixing <= 10 THEN 'RI1 [<10]' WHEN DD.saldohoy * TCA.fixing <= 100 THEN 'RI2 <10 - 100]' WHEN DD.saldohoy * TCA.fixing <= 300 THEN 'RI3 <100 - 300]' WHEN DD.saldohoy * TCA.fixing <= 500 THEN 'RI4 <300 - 500]' WHEN DD.saldohoy * TCA.fixing <= 1000 THEN 'RI5 <500 - 1000]' WHEN DD.saldohoy * TCA.fixing <= 2000 THEN 'RI6 <1000 - 2000]' WHEN DD.saldohoy * TCA.fixing <= 5000 THEN 'RI7 <2000 - 5000]' WHEN DD.saldohoy * TCA.fixing <= 10000 THEN 'RI8 <5000 - 10000]' WHEN DD.saldohoy * TCA.fixing <= 25000 THEN 'RI9 <10000 - 25000]' WHEN DD.saldohoy * TCA.fixing <= 50000 THEN 'RI10 <25000 - 50000]' WHEN DD.saldohoy * TCA.fixing <= 100000 THEN 'RI11 <50000 - 100000]' WHEN DD.saldohoy * TCA.fixing > 100000 THEN 'RI12 [>100000]' END AS RangoImpago," &_
                        "convert(varchar,DD.fechaincumplimiento,103) AS fechaincumplimiento," &_
                        ...
                        "CASE WHEN DD.fono1<>'' THEN DD.tipofono1 + '-' + DD.prefijo1 + '-' + DD.fono1 + (CASE WHEN LTRIM(RTRIM(DD.extension1))<>'' and LTRIM(RTRIM(DD.extension1))<>'0000' THEN ' - ' + LTRIM(RTRIM(DD.extension1)) ELSE '' END ) ELSE '' END AS telefono," &_
                        "CASE WHEN DD.direccion <>'' THEN rtrim(ltrim(DD.direccion)) + ' - ' + rtrim(ltrim(UBI2.departamento)) + ' - ' + rtrim(ltrim(UBI2.provincia)) + ' - ' + rtrim(ltrim(UBI2.distrito)) ELSE ' ' END as direccion," &_
                        "AG.razonsocial as Agencia, " &_
                        "convert(varchar,RG.fhgestionado,103) + ' ' + convert(varchar,RG.fhgestionado,108) AS fhGestionado, " &_
                        "IsNull(TC.descripcion,'') as TipoContacto," &_
                        "GS.CodGestion + '-' + GS.descripcion as Gestion," &_
                        "IsNull(convert(varchar,RG.FechaPromesa,103),'') AS fechapromesa," &_
                        ...
                        "CASE WHEN RG.direccion <>'' THEN rtrim(ltrim(RG.direccion)) + '-' + rtrim(ltrim(UBI.departamento)) + '-' + rtrim(ltrim(UBI.provincia)) + '-' + rtrim(ltrim(UBI.distrito)) ELSE ' ' END as direcciongestion,DD.Nro " &_
                        " into ##Userexp" & session("codusuariodif") & " from " &_
                        ...
                        sql="select convert(varchar,DD.FechaGestion,103) AS FechaAsig, " &_
		                        ...
		                        "CASE WHEN DD.direccion <>'' THEN rtrim(ltrim(DD.direccion)) + ' - ' + rtrim(ltrim(UBI2.departamento)) + ' - ' + rtrim(ltrim(UBI2.provincia)) + ' - ' + rtrim(ltrim(UBI2.distrito)) ELSE ' ' END as direccion, " &_
		                        "DD.Agencia, " &_
		                        "convert(varchar,DD.fhgestionado,103) + ' ' + convert(varchar,DD.fhgestionado,108) AS fhGestionado, " &_
		                        "IsNull(DD.TipoContacto,'') as TipoContacto," &_
		                        "DD.CodGestion + '-' + DD.Gestion as Gestion," &_
		                        "IsNull(convert(varchar,DD.FechaPromesa,103),'') AS fechapromesa," &_
		                        "CASE WHEN DD.fono<>'' THEN DD.tipofono + '-' + DD.prefijo + '-' + DD.fono + (CASE WHEN LTRIM(RTRIM(DD.extension))<>'' and LTRIM(RTRIM(DD.extension))<>'0000' THEN '-' + LTRIM(RTRIM(DD.Extension)) END ) ELSE ' ' END AS telefonoGestion," &_
		                        "CASE WHEN DD.RGdireccion <>'' THEN rtrim(ltrim(DD.RGdireccion)) + ' - ' + rtrim(ltrim(UBI.departamento)) + ' - ' + rtrim(ltrim(UBI.provincia)) + ' - ' + rtrim(ltrim(UBI.distrito)) ELSE ' ' END as direcciongestion," &_
		                        "DD.Nro " &_
                        "into ##Userexp" & session("codusuariodif") & " " &_
                        "from ##User" & session("codusuariodif") & " DD " &_
                        "left outer join Territorio TE ON DD.CodTerritorio=TE.codterritorio " &_
                        "left outer join Oficina OFI ON DD.codoficina=OFI.codoficina " &_
                        "left outer join TipoCambio TCA ON DD.fechadatos=TCA.fechadatos AND DD.Divisa=TCA.divisa and TCA.tipo='S' " &_
                        "left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 " &_
                        "left outer join Marca MA ON MC.codmarca=MA.codmarca " &_
                        "left outer join Ubigeo UBI on DD.RGcoddpto=UBI.coddpto and DD.RGcoddist=UBI.coddist and DD.RGcodprov=UBI.codprov " &_
                        "left outer join Ubigeo UBI2 on DD.codestado=UBI2.coddpto and DD.coddistrito=UBI2.coddist and DD.codprovincia=UBI2.codprov"
                        

                       sql="select " &_
		                        "convert(varchar,DD.FechaGestion,103) AS FechaAsig, " &_
		                        "char(39) + DD.contrato AS contrato, " &_
		                        "char (39) + DD1.codigocentral AS codigocentral, " &_
		                        "DD.nombre, " &_
		                        "char(39) + DD1.codterritorio as codterritorio, " &_
		                        "TE.descripcion AS Territorio, " &_
		                        "char(39) + DD1.codoficina as codoficina, " &_
		                        "OFI.descripcion AS oficina, " &_
		                        "DD.Producto, " &_
		                        "DD.SubProducto, " &_
		                        "DD.Divisa, " &_
		                        ...
		                        "CASE WHEN DD.divisa <>'PEN' THEN DD.SaldoHoy*TCA.fixing ELSE DD.SaldoHoy END AS SaldoHoySoles, " &_
		                        "CASE WHEN DD.saldohoy*TCA.fixing <= 10 THEN 'RI1 [<10]' WHEN DD.saldohoy * TCA.fixing <= 100 THEN 'RI2 <10 - 100]' WHEN DD.saldohoy * TCA.fixing <= 300 THEN 'RI3 <100 - 300]' WHEN DD.saldohoy * TCA.fixing <= 500 THEN 'RI4 <300 - 500]' WHEN DD.saldohoy * TCA.fixing <= 1000 THEN 'RI5 <500 - 1000]' WHEN DD.saldohoy * TCA.fixing <= 2000 THEN 'RI6 <1000 - 2000]' WHEN DD.saldohoy * TCA.fixing <= 5000 THEN 'RI7 <2000 - 5000]' WHEN DD.saldohoy * TCA.fixing <= 10000 THEN 'RI8 <5000 - 10000]' WHEN DD.saldohoy * TCA.fixing <= 25000 THEN 'RI9 <10000 - 25000]' WHEN DD.saldohoy * TCA.fixing <= 50000 THEN 'RI10 <25000 - 50000]' WHEN DD.saldohoy * TCA.fixing <= 100000 THEN 'RI11 <50000 - 100000]' WHEN DD.saldohoy * TCA.fixing > 100000 THEN 'RI12 [>100000]' END AS RangoImpago, " &_
		                        "convert(varchar,DD1.fechaincumplimiento,103) AS fechaincumplimiento, " &_
		                        "DD.DiasVencimiento, " &_
		                        "CASE WHEN DD.DiasVencimiento <= 30 THEN 'T1 [1-30]' WHEN DD.DiasVencimiento <= 60 THEN 'T2 [31-60]' WHEN DD.DiasVencimiento <= 90 THEN 'T3 [61-90]' WHEN DD.DiasVencimiento <= 120 THEN 'T4 [91-120]' END AS TramoReal, " &_
		                        "MA.descripcion AS Marca, " &_
		                        "CASE WHEN DD.fono1<>'' THEN DD.tipofono1 + '-' + DD.prefijo1 + '-' + DD.fono1 + (CASE WHEN LTRIM(RTRIM(DD.extension1))<>'' and LTRIM(RTRIM(DD.extension1))<>'0000' THEN '-' + LTRIM(RTRIM(DD.extension1)) ELSE '' END ) ELSE '' END AS telefono, " &_
		                        "CASE WHEN DD1.direccion <>'' THEN rtrim(ltrim(DD1.direccion)) + ' - ' + rtrim(ltrim(UBI2.departamento)) + ' - ' + rtrim(ltrim(UBI2.provincia)) + ' - ' + rtrim(ltrim(UBI2.distrito)) ELSE ' ' END as direccion, " &_
		                        "DD.Agencia, " &_
		                        "convert(varchar,DD.fhgestionado,103) + ' ' + convert(varchar,DD.fhgestionado,108) AS fhGestionado, " &_
		                        "IsNull(DD.TipoContacto,'') as TipoContacto, " &_
		                        "DD.CodGestion + '-' + DD.Gestion as Gestion, " &_
		                        "IsNull(convert(varchar,RG.FechaPromesa,103),'') AS fechapromesa, " &_
		                        "CASE WHEN RG.fono<>'' THEN RG.tipofono + '-' + RG.prefijo + '-' + RG.fono + (CASE WHEN LTRIM(RTRIM(RG.extension))<>'' and LTRIM(RTRIM(RG.extension))<>'0000' THEN '-' + LTRIM(RTRIM(RG.Extension)) END ) ELSE ' ' END AS telefonoGestion, " &_
		                        "CASE WHEN RG.direccion <>'' THEN rtrim(ltrim(RG.direccion)) + ' - ' + rtrim(ltrim(UBI.departamento)) + ' - ' + rtrim(ltrim(UBI.provincia)) + ' - ' + rtrim(ltrim(UBI.distrito)) ELSE ' ' END as direcciongestion, " &_
		                        "DD.Nro  " &_
                        "into ##Userexp" & session("codusuariodif") & " " &_
                        "from ##User" & session("codusuariodif") & " DD " &_
                        "inner join BusquedaDistribucion DD1 on DD.codigocentral=DD1.codigocentral and DD.contrato=DD1.contrato and DD.fechadatos=DD1.fechadatos " &_
                        "left outer join Territorio TE ON DD1.CodTerritorio=TE.codterritorio " &_
                        "left outer join Oficina OFI ON DD1.codoficina=OFI.codoficina " &_
                        "left outer join TipoCambio TCA ON DD.fechadatos=TCA.fechadatos AND DD.Divisa=TCA.divisa and TCA.tipo='S' " &_
                        "left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 " &_
                        "left outer join Marca MA ON MC.codmarca=MA.codmarca " &_
                        "left outer join RespuestaGestion RG on DD.codigocentral=RG.codigocentral and DD.contrato=RG.contrato and DD1.mejor_codrespgestion=RG.codrespgestion " &_
                        "left outer join Gestion GS ON RG.codgestion = GS.codgestion " &_
                        "left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " &_
                        "left outer join Ubigeo UBI on RG.coddpto=UBI.coddpto and RG.coddist=UBI.coddist and RG.codprov=UBI.codprov " &_
                        "left outer join Ubigeo UBI2 on DD1.codestado=UBI2.coddpto and DD1.coddistrito=UBI2.coddist and DD1.codprovincia=UBI2.codprov"
                        ''No es necesario el filtro porque se toma la tabla temporal del reporte filtrada
                    ...
                    sql="select convert(varchar,DD.FechaGestion,103) AS FechaAsig, " &_
                        ...
                        "IsNull(convert(varchar,RG.FechaPromesa,103),'') AS fechapromesa," &_
                        "CASE WHEN RG.fono<>'' THEN RG.tipofono + '-' + RG.prefijo + '-' + RG.fono + (CASE WHEN LTRIM(RTRIM(RG.extension))<>'' and LTRIM(RTRIM(RG.extension))<>'0000' THEN '-' + LTRIM(RTRIM(RG.Extension)) END )ELSE ' ' END AS telefonoGestion," &_
                        "CASE WHEN RG.direccion <>'' THEN rtrim(ltrim(RG.direccion)) + '-' + rtrim(ltrim(UBI.departamento)) + '-' + rtrim(ltrim(UBI.provincia)) + '-' + rtrim(ltrim(UBI.distrito)) ELSE ' ' END as direcciongestion,CASE WHEN len(rtrim(ltrim(replace(replace(RG.comentario,char(10),''),char(13),''))))<=100 THEN rtrim(ltrim(replace(replace(RG.comentario,char(10),''),char(13),''))) ELSE substring(rtrim(ltrim(replace(replace(RG.comentario,char(10),''),char(13),''))),1,100) + '...' END as Comentario,DD.Nro " &_
                        " into ##Userexp" & session("codusuariodif") & " from " &_
                        "(select * from BusquedaDistribucion where Nro in (select Nro from ##User" & session("codusuariodif") & ")) DD  " &_
                        "inner join Agencia AG on DD.codagencia=AG.codagencia " &_
                        "left outer join Producto PR on DD.codproducto=PR.codproducto " &_
                        "left outer join SubProducto SP on DD.codsubproducto=SP.codsubproducto " &_
                        "left outer join FamProducto FP ON SP.codfamproducto=FP.codfamproducto " &_
                        "left outer join Oficina OFI ON DD.codoficina=OFI.codoficina " &_
                        ...
                        "left outer join MarcaCliente MC ON DD.codigocentral=MC.codigocentral and MC.activo=1 " &_
                        "left outer join Marca MA ON MC.codmarca=MA.codmarca " &_
                        "left outer join Ubigeo UBI2 on DD.codestado=UBI2.coddpto and DD.coddistrito=UBI2.coddist and DD.codprovincia=UBI2.codprov " &_
                        "left outer join RespuestaGestion RG on DD.mejor_codrespgestion=RG.codrespgestion " &_
                        "left outer join Gestion GS ON RG.codgestion = GS.codgestion " &_
                        "left outer join TipoContacto TC ON GS.codtipocontacto = TC.codtipocontacto " &_
                        "left outer join Ubigeo UBI on RG.coddpto=UBI.coddpto and RG.coddist=UBI.coddist and RG.codprov=UBI.codprov "                        
                        ''"where DD.Nro in (select A.Nro from ##User" & session("codusuariodif") & " A)"
                        ''"BusquedaDistribucion DD inner join Agencia AG on DD.codagencia=AG.codagencia " &_
                        ''No es necesario el filtro porque se toma la tabla temporal del reporte filtrada                            
                        
                        
                    sql="select * into ##Userexp" & session("codusuariodif") & " from VISTAEXPDISTRIBUCION where Nro in (select Nro from ##User" & session("codusuariodif") & ")"
		            ''response.write "<!--" & sql & "-->"
		            ''response.write "<!--" & filtroDD & "-->"
		            ''response.write "<!--" & filtroFP & "-->"
		            ''response.write "<!--" & filtroRG & "-->"
		            ''response.write "<!--" & filtroMC & "-->"
		            ''response.Flush()
		            conn.execute sql                 
                   ...
                   '' RS.Close
					...
					consulta_exp="select 'Fecha Asignación','N° Contrato','Cod.Central','Nombre','Cod.Territorio','Territorio','Cod.Oficina','Oficina','Producto','Subprod','Moneda','Deuda','Deuda en soles','Rango Impago','Fecha Incumplimiento','Dias Atraso','Tramo Real','Marca','Teléfono','Dirección','Agencia','F.Gest','Tipo Contacto','Mejor Gestión','F.promesa','Teléfono Gestión','Dirección Gestión','Comentario'"
                    ...
                    consulta_exp="select A.FechaAsig,A.contrato,A.codigocentral,A.nombre,A.codterritorio,A.Territorio,A.codoficina,A.oficina,A.Producto,A.SubProducto,A.Divisa,A.SaldoHoy,A.SaldoHoySoles,A.RangoImpago,A.fechaincumplimiento,A.DiasVencimiento,A.TramoReal,A.Marca,A.telefono,A.direccion,A.Agencia,A.fhGestionado,A.TipoContacto,A.Gestion,A.fechapromesa,A.telefonoGestion,A.direcciongestion,A.comentario " & _
								 "from tempdb.dbo.##Userexp" & session("codusuariodif") & " A order by A.Nro"
					...
					sql="drop table tempdb.dbo.##Userexp" & session("codusuariodif")
                    conn.execute sql								
				%>
					<script language="javascript">
