...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("cargaemail.asp") then
			'###############################################
					...
					For Each File In Uploader.Files.Items		
						...
						if bolUpload = true then
							if cuentaarch=0 then
								...
								sql="INSERT INTO CargaArchivo(proceso,rutaarchivo,mailinforme,usuarioregistra,fecharegistra,codestadocarga) VALUES ('emails','" & RutaFisicaUpload & "\emails','" & correo & "'," & session("codusuario") & ",getdate(),1)"
								conn.execute sql		
								
								''capturamos el maximo CargaArchivo para el FileName que sera "emails(nro).txt"
								...
								File.FileName="emails" & codcarga & "." & GetFileExtension(File.FileName)
			...
			''Response.Redirect "cargaemail.asp?agregardato=" & Uploader.Form("agregardato") & "&flaginactivar=" & Uploader.Form("flaginactivar") & "&archivotxt=" & strMessage
			%>
				<script language=javascript>
					window.open("cargaemail.asp?agregardato=<%=Uploader.Form("agregardato")%>&flaginactivar=<%=Uploader.Form("flaginactivar")%>&archivotxt=<%=strMessage%>","_self");
				</script>
			<%
