...
if session("codusuario")<>"" then
	conectar
	if permisofacultad("admmarca.asp") then
	...
	%>
		...
		function agregar()
		{
			ventanamarcas=global_popup_IWTSystem(ventanamarcas,"nuevomarcas.asp?vistapadre=" + window.name + "&paginapadre=admmarcas.asp","Newmarca","scrollbars=no,scrolling=no,top=" + ((screen.height - 140)/2 - 30) + ",height=140,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    anchocolumna[tabla] =  new Array(  '5%' , '15%' ,'5%' , '');
		    ...
		    objetofomulario[tabla] = new Array();
				...
				objetofomulario[tabla][1]='<input type=hidden name=codmarca-id- value=-c0->' + '<a href="javascript:modificar(-id-);">-valor-</a>';
		...
		contadortotal=0
		sql="select count(*) from marca  " & filtrobuscador 
		consultar sql,RS	
		...
		sql="select top " & cantidadxpagina & " * " & _
			"from marca where " & filtrobuscador1 & " codmarca not in (select top " & topnovisible & " codmarca from marca " & filtrobuscador & " order by codmarca) order by codmarca" 
		else
		sql="select top " & cantidadxpagina & " * " & _
			"from marca " & filtrobuscador & " order by codmarca" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codmarca" & RS.Fields("codmarca"))<>"" then
					
					
					if obtener("activo" & RS.Fields("codmarca")) <> "" then
						activo="1"
					else
						activo="0"
					end if											
						...
						if 	int(activo)<> rs.Fields("activo") then
							sql="update marca set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codmarca= '" & rs.Fields("codmarca") &  "'" 
							''response.write sql
							conn.Execute sql
						end if
		...
		%>
			...
			datos[tabla][<%=contador%>][0]=<%=RS.Fields("codmarca")%>;
			datos[tabla][<%=contador%>][1]='<%=rs.Fields("descripcion")%>';
			datos[tabla][<%=contador%>][2]=<%if obtener("actualizarlista")<>"" and obtener("codmarca" & RS.Fields("codmarca"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Marcas (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Marcas (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.Marca','Descripción','Activo' "
					...
					consulta_exp="select codmarca,descripcion,CASE WHEN Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
