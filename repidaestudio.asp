			...
			<form name=formula method=post>
					...
					<%
                        ...
                        sql="select estudio,SUM(jud) as StockCierre, " &_
                            ...
                            "from FB.CentroCobranzas.dbo.PD_Casos_Jud A where SEGMENTO_RIESGO in ('PARTICULARES','PYME') and fechadatos=(select MAX(fechacierre) from FB.CentroCobranzas.dbo.PD_FechasCierre where fechacierre<'" & mid(fechacierre,7,4) & mid(fechacierre,4,2) & mid(fechacierre,1,2) & "') group by estudio order by StockCierre desc"
                        bgcolor="#FFFFFF"
                        ''response.Write sql
                        consultar sql,RS	
                        ...
                        <tr>
	                        ...
	                        <td bgcolor="<%=bgcolor%>" align="right" width="5%">
		                        <a href="repsaecasosdet.asp?paginapadre=repidaestudio.asp&asignado=&fechacierre=<%=fechacierre%>&codterritorio=&plaza=&segmento=&confase="><font size=1 color=#483d8b face=Arial><%=FormatNumber(iif(RS.Fields("StockCierre")+RS.Fields("Ingresos")>0,RS.Fields("Salidas")*100/(RS.Fields("StockCierre")+RS.Fields("Ingresos")),0),2)%>%</font></a>
	                        </td>	
