...
if session("codusuario")<>"" then
	...
	%>
		...
		{
		    ventanafamproducto = global_popup_IWTSystem(ventanafamproducto, "nuevofamproducto.asp?vistapadre=" + window.name + "&paginapadre=admfamproducto.asp&codfamproducto=" + codigo, "NewfamProducto", "scrollbars=yes,scrolling=yes,top=" + ((screen.height - 180) / 2 - 30) + ",height=180,width=" + (screen.width / 2 - 10) + ",left=" + (screen.width / 4) + ",resizable=yes");
		}			
		function agregar()
		{
		    ventanafamproducto = global_popup_IWTSystem(ventanafamproducto, "nuevofamproducto.asp?vistapadre=" + window.name + "&paginapadre=admfamproducto.asp", "NewfamProducto", "scrollbars=yes,scrolling=yes,top=" + ((screen.height - 180) / 2 - 30) + ",height=180,width=" + (screen.width / 2 - 10) + ",left=" + (screen.width / 4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Codigo','Descripción', 'Producto','Activo','Editar');
		    ...
		    aligncabecera[tabla] = new Array('center','left','left','center','left');
		    aligndetalle[tabla] = new Array('center','left','left','center','left');
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codfamproducto-id- value=-c0->' + '<a href=javascript:modificar("-id-");>-valor-</a>';
		...
		<%
		if buscador<>"" then
			filtrobuscador = " where (f.codfamproducto like '%" & buscador & "%' or f.descripcion like '%" & buscador & "%' or g.descripcion like '%" & buscador & "%') "
		end if
		...
		contadortotal=0
		sql="select count(*) from famproducto f left outer join producto g on f.codproducto = g.codproducto " & filtrobuscador
		consultar sql,RS	
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " *,f.descripcion as descrip,g.descripcion as grupo, f.activo as activo from famproducto f left outer join producto g on f.codproducto = g.codproducto where " & filtrobuscador1 & " f.codfamproducto not in (select top " & topnovisible & " f.codfamproducto from famproducto f left outer join producto g on f.codproducto = g.codproducto " & filtrobuscador & " order by f.codfamproducto) order by f.codfamproducto)" 
		else
		sql="select top " & cantidadxpagina & " *,f.descripcion as descrip,g.descripcion as grupo, f.activo  as activo from famproducto f left outer join producto g on f.codproducto = g.codproducto " & filtrobuscador & " order by f.codfamproducto" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codfamproducto" & RS.Fields("codfamproducto"))<>"" then
					
					if obtener("activo" & RS.Fields("codfamproducto"))<>"" then
						activo="1"
					else
						activo="0"
					end if											
						...
						if 	int(activo)<> rs.Fields("activo") then
							sql="update famproducto set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codfamproducto= '" & rs.Fields("codfamproducto")&  "'" 
							''response.write sql
							conn.Execute sql
						end if
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]=<%=RS.Fields("codfamproducto")%>;
				datos[tabla][<%=contador%>][1]='<%=iif(isNull(rs.Fields("descrip")),"",rs.Fields("descrip"))%>';
				datos[tabla][<%=contador%>][2]='<%=rs.Fields("grupo")%>';
				datos[tabla][<%=contador%>][3]=<%if obtener("actualizarlista")<>"" and obtener("codfamproducto" & RS.Fields("codfamproducto"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Familia Producto (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Familia Producto (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.FamProducto','Descripción','Producto','Activo'"
					...
					consulta_exp="select f.codfamproducto,f.descripcion,g.descripcion, CASE WHEN f.Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.famproducto f left outer join CobranzaCM.dbo.producto g on f.codproducto = g.codproducto  " & filtrobuscador & " order by f.codfamproducto" 
