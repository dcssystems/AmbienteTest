...
if session("codusuario")<>"" then
	...
	%>
		...
		{
			ventanagestion=global_popup_IWTSystem(ventanagestion,"nuevogestion.asp?vistapadre=" + window.name + "&paginapadre=admgestion.asp&codgestion=" + codigo,"Newgestion","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}			
		function agregar()
		{
			ventanagestion=global_popup_IWTSystem(ventanagestion,"nuevogestion.asp?vistapadre=" + window.name + "&paginapadre=admgestion.asp","Newgestion","scrollbars=yes,scrolling=yes,top=" + ((screen.height - 200)/2 - 30) + ",height=200,width=" + (screen.width/2 - 10) + ",left=" + (screen.width/4) + ",resizable=yes");
		}
		function actualizar()
		{
		...
		<script language=javascript>
		  ...
		  //Configuración general de datos de tabla 0
		    ...
		    cabecera[tabla] = new Array('Código','Descripción','Acción','Contacto','Act','Editar');
		    identificadorfilas[tabla]="fila";
		    pievisible[tabla]=true;
		    columnavisible[tabla] = new Array(true, true, true ,true,true,true);
		    anchocolumna[tabla] =  new Array( '5%', '50%', '10%','15%' , '3%' ,'');
		    ...
		    objetofomulario[tabla] = new Array();
				objetofomulario[tabla][0]='<input type=hidden name=codgestion-id- value=-c0->' + '<a href=javascript:modificar("-id-");>-valor-</a>';
		...
		<%
		if buscador<>"" then
			filtrobuscador = " where (f.codgestion like '%" & buscador & "%' or f.descripcion like '%" & buscador & "%' or g.descripcion like '%" & buscador & "%' or h.descripcion like '%" & buscador & "%') "
		end if
		...
		contadortotal=0
		sql="select count(*) from gestion f inner join tipoaccion g on f.codtipoaccion = g.codtipoaccion inner join TipoContacto h on f.codtipocontacto = h.codtipocontacto" & filtrobuscador 
		consultar sql,RS	
		...
		if pag>1 then					
		sql="select top " & cantidadxpagina & " *,f.descripcion as descrip,g.descripcion as tipaccion, h.descripcion as tipcontacto, f.activo as activo from gestion f inner join tipoaccion g on f.codtipoaccion = g.codtipoaccion inner join TipoContacto h on f.codtipocontacto = h.codtipocontacto where " & filtrobuscador1 & " f.codgestion not in (select top " & topnovisible & " f.codgestion from gestion f inner join tipoaccion g on f.codtipoaccion = g.codtipoaccion inner join TipoContacto h on f.codtipocontacto = h.codtipocontacto " & filtrobuscador & " order by f.codgestion) order by f.codgestion" 
		else
		sql="select top " & cantidadxpagina & " *,f.descripcion as descrip,g.descripcion as tipaccion, h.descripcion as tipcontacto, f.activo  as activo from gestion f inner join tipoaccion g on f.codtipoaccion = g.codtipoaccion inner join TipoContacto h on f.codtipocontacto = h.codtipocontacto " & filtrobuscador & " order by f.codgestion" 
		end if
		''response.write sql
		consultar sql,RS
		contador=0
			...
			Do while not RS.EOF 
				if obtener("actualizarlista")<>"" and obtener("codgestion" & RS.Fields("codgestion"))<>"" then
					
					if obtener("activo" & RS.Fields("codgestion"))<>"" then
						activo="1"
					else
						activo="0"
					end if											
						...
						if 	int(activo)<> rs.Fields("activo") then
							sql="update gestion set activo=" & activo & ",usuariomodifica=" & session("codusuario") & ",fechamodifica=getdate() where codgestion= '" & rs.Fields("codgestion")&  "'" 
							''response.write sql
							conn.Execute sql
						end if
		...
		%>
			datos[tabla][<%=contador%>] = new Array();
				datos[tabla][<%=contador%>][0]='<%=RS.Fields("codgestion")%>';
				datos[tabla][<%=contador%>][1]='<%=iif(isNull(rs.Fields("descrip")),"",rs.Fields("descrip"))%>';
				datos[tabla][<%=contador%>][2]='<%=rs.Fields("tipaccion")%>';
				datos[tabla][<%=contador%>][3]='<%=rs.Fields("tipcontacto")%>';
				datos[tabla][<%=contador%>][4]=<%if obtener("actualizarlista")<>"" and obtener("codgestion" & RS.Fields("codgestion"))<>"" then%><%if int(activo)=1 then%>'checked'<%else%>' '<%end if%><%else%><%if rs.Fields("activo")=1 then%>'checked'<%else%>' '<%end if%><%end if%>;
		...
		<%if contador=0 then%>
			...
			<tr>
				<td bgcolor="#F5F5F5"><font size=2 face=Arial color=#00529B><b>Gestión (0) - No hay registros.</b></font>&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a></td>
		...
		%>
			...
			<tr>
				<td bgcolor="#F5F5F5" align=left><font size=2 face=Arial color=#00529B><b>Gestión (<%=contadortotal%>)&nbsp;&nbsp;<a href="javascript:actualizar();"><img src="imagenes/guardar.gif" border=0 alt="Guardar" title="Guardar" align=middle></a>&nbsp;&nbsp;<a href="javascript:agregar();"><img src="imagenes/nuevo.gif" border=0 alt="Nuevo" title="Nuevo" align=middle></a>&nbsp;&nbsp;<a href="javascript:exportar();"><img src="imagenes/excel.gif" border=0 alt="Exportar a Excel" title="Exportar a Excel" align=middle></a><!--&nbsp;&nbsp;<a href="javascript:imprimir();"><img src="imagenes/imprimir.gif" border=0 alt="Imprimir" title="Imprimir" align=middle></a>--><%if expimp="1" then%>&nbsp;&nbsp;<a href='<%=RutaWebExportar%>/UserExport<%=session("codusuario")%>.xls?time=<%=tiempoexport%>','_self'><img src="imagenes/descargar.gif" border=0 alt="Descargar Excel" title="Descargar Excel" align=middle></a><%end if%></b></font></td>
				<td bgcolor="#F5F5F5" align=right><%=objetosdebusqueda%></td>
				<td bgcolor="#F5F5F5" align=left><a href="javascript:buscar();"><img src="imagenes/buscar.gif" border=0 alt="Buscar" title="Buscar" align=middle></a></td>
		...
		''Si se pide exportar a excel
				if expimp="1" then
					...
					consulta_exp="select 'Cod.Gestión','Descripcion','Acción','Contacto','Activo' "
					...
					consulta_exp="select char(39) + f.codgestion,f.descripcion,g.descripcion,h.descripcion,CASE WHEN f.Activo=1 THEN 'Sí' ELSE 'No' END as FlagActivo " & _
								 "from CobranzaCM.dbo.gestion f inner join CobranzaCM.dbo.tipoaccion g on f.codtipoaccion = g.codtipoaccion inner join CobranzaCM.dbo.TipoContacto h on f.codtipocontacto = h.codtipocontacto " & filtrobuscador & " order by f.codgestion " 
