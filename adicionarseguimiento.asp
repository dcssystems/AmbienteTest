...
%>
		...
		<html>
			...
			<script language=javascript>
				...
				var textocodgestion;
				textocodgestion = document.formula.codgestion.options[document.formula.codgestion.selectedIndex].innerText;
				if (textocodgestion==undefined) { textocodgestion = document.formula.codgestion.options[document.formula.codgestion.selectedIndex].textContent; }
				
				if (textocodgestion.toUpperCase().replace('PROMESA','').length!=textocodgestion.length)
				{
				...
				{
				document.formula.action="adicionarseguimiento.asp";
...
function showCalendar(id, format, showsTime, showsOtherMonths) {
  ...
  return false;
			...
			<body topmargin=0 leftmargin=0 bgcolor="#FFFFFF">
				<form name=formula method=post action="adicionarseguimiento.asp">
					...
					<td>
						...
						if codtipoaccion="2" then ''visita
							...
							sql="select top 1 A.direccion,B.departamento,B.provincia,B.distrito from " & vistabusqueda & " A inner join Ubigeo B on A.codestado=B.coddpto and A.coddistrito=B.coddist and A.codprovincia=B.codprov where A.codigocentral='" & codigocentral & "' and fechadatos='" & mid(obtener("fechadatos"),7,4) & mid(obtener("fechadatos"),4,2) & mid(obtener("fechadatos"),1,2) & "'"
							''Response.Write sql
							consultar sql,RS1	
							if not RS1.EOF then
							...
							if codtipoaccion<>"2" then ''no visita
								''sql="select max(convert(varchar,fechagestion,112)) from BusquedaDistribucion"
								''consultar sql,RS	
								''maxfechagestion=rs.fields(0)
								''RS.Close	
