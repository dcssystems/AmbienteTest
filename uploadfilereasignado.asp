...
if session("codusuario")<>"" then
	...
	if permisofacultad("admreasignado.asp") then
						...
						if bolUpload = true then
							if cuentaarch=0 then
								...
								if Uploader.Form("activo")<>"" then ''Estado Pendiente actualización total
									sql="INSERT INTO CargaArchivo(proceso,rutaarchivo,mailinforme,usuarioregistra,fecharegistra,codestadocarga) VALUES ('Reasignados','" & RutaFisicaUpload & "\reasignados','" & correo & "'," & session("codusuario") & ",getdate(),2)"
								else ''Estado Pendiente actualización inserción
									sql="INSERT INTO CargaArchivo(proceso,rutaarchivo,mailinforme,usuarioregistra,fecharegistra,codestadocarga) VALUES ('Reasignados','" & RutaFisicaUpload & "\reasignados','" & correo & "'," & session("codusuario") & ",getdate(),1)"
								end if
								conn.execute sql		
								
								''capturamos el maximo CargaArchivo para el FileName que sera "reasignados(nro).txt"
								...
								File.FileName="reasignados" & codcarga & "." & GetFileExtension(File.FileName)
			...
			''Response.Redirect "cargarreasignado.asp?agregardato=" & Uploader.Form("agregardato") & "&flaginactivar=" & Uploader.Form("flaginactivar") & "&archivotxt=" & strMessage
			%>
				<script language=javascript>
					window.open("cargarreasignado.asp?agregardato=<%=Uploader.Form("agregardato")%>&flaginactivar=<%=Uploader.Form("flaginactivar")%>&archivotxt=<%=strMessage%>","_self");
				</script>
			<%
